...
namespace CalamityModClassicPreTrailer.BiomeManagers
{
	...
	{ 
		...
		public bool MeetsBaseAbyssRequirement(Player player)
		{
			...
			int abyssChasmX = (CalamityWorldPreTrailer.abyssSide
				? genLimit - (genLimit - 135)
				: genLimit + (genLimit - 135));
		...
		public override bool IsBiomeActive(Player player)
		{
			Point point = player.Center.ToTileCoordinates();
			int y = Main.maxTilesY;
			return MeetsBaseAbyssRequirement(player) && (double)point.Y > (Main.rockLayer + (double)y * 0.14) &&
