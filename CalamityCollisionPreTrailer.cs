...
{
	public class CalamityCollisionPreTrailer
	{
		public static bool HotWetCollision(Vector2 Position, int Width, int Height)
		{
			...
			for (int i = arg_DB_0; i < num3; i++)
			{
				for (int j = num4; j < num5; j++)
				{
					...
					{
						if ((Main.tile[i, j + 2].LiquidType == LiquidID.Lava) ||
							(							Main.tile[i, j + 3].LiquidType == LiquidID.Lava) ||
							(							Main.tile[i, j + 4].LiquidType == LiquidID.Lava))
						{
							if (Main.tile[i, j].LiquidAmount > 0)
							{
								...
								int num6 = 16;
								float num7 = (float)(256 - (int)Main.tile[i, j].LiquidAmount);
								num7 /= 32f;
								vector2.Y += num7 * 2f;
								num6 -= (int)(num7 * 2f);
							...
							else if (Main.tile[i, j].HasTile && Main.tile[i, j].Slope != 0 && j > 0 && Main.tile[i, j - 1] != null && Main.tile[i, j - 1].LiquidAmount > 0)
							{
								Vector2 vector2;
								vector2.X = (float)(i * 16);
								vector2.Y = (float)(j * 16);
