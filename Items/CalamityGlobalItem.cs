...
namespace CalamityModClassicPreTrailer.Items
{
	public class CalamityGlobalItem : GlobalItem
	{
		...
		public override void SetDefaults(Item item)
		{
			...
			if (CalamityModClassicPreTrailer.weaponAutoreuseList.Contains(item.type))
				item.autoReuse = true;

			if (item.type == ItemID.PsychoKnife)
				item.damage *= 4;
			else if (CalamityModClassicPreTrailer.doubleDamageBuffList.Contains(item.type))
				item.damage *= 2;
			else if (CalamityModClassicPreTrailer.sixtySixDamageBuffList.Contains(item.type))
				item.damage = (int)((double)item.damage * 1.66);
			else if (CalamityModClassicPreTrailer.fiftyDamageBuffList.Contains(item.type))
				item.damage = (int)((double)item.damage * 1.5);
			else if (CalamityModClassicPreTrailer.thirtyThreeDamageBuffList.Contains(item.type))
				item.damage = (int)((double)item.damage * 1.33);
			else if (CalamityModClassicPreTrailer.twentyFiveDamageBuffList.Contains(item.type))
				item.damage = (int)((double)item.damage * 1.25);
			else if (CalamityModClassicPreTrailer.twentyDamageBuffList.Contains(item.type))
			...
			else if (CalamityModClassicPreTrailer.quarterDamageNerfList.Contains(item.type))
		...
		public override bool Shoot(Item item, Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
		{
			if (rogue)
			{
				velocity.X *= CalamityCustomThrowingDamagePlayer.ModPlayer(player).throwingVelocity;
				velocity.Y *= CalamityCustomThrowingDamagePlayer.ModPlayer(player).throwingVelocity;
			}
			if (player.GetModPlayer<CalamityPlayerPreTrailer>().luxorsGift)
			{
				if (player.whoAmI == Main.myPlayer)
				{
					if (item.CountsAsClass(DamageClass.Melee))
					{
						Projectile.NewProjectile(source, position.X, position.Y, velocity.X * 0.5f, velocity.Y * 0.5f, Mod.Find<ModProjectile>("LuxorsGiftMelee").Type, (int)((double)damage * 0.6), 0f, player.whoAmI, 0f, 0f);
					}
					else if (rogue)
					{
						Projectile.NewProjectile(source, position.X, position.Y, velocity.X, velocity.Y, Mod.Find<ModProjectile>("LuxorsGiftRogue").Type, (int)((double)damage * 0.8), 0f, player.whoAmI, 0f, 0f);
					}
					else if (item.CountsAsClass(DamageClass.Ranged))
					{
						Projectile.NewProjectile(source, position.X, position.Y, velocity.X * 1.5f, velocity.Y * 1.5f, Mod.Find<ModProjectile>("LuxorsGiftRanged").Type, (int)((double)damage * 0.5), 0f, player.whoAmI, 0f, 0f);
					}
					else if (item.CountsAsClass(DamageClass.Magic))
					{
						Projectile.NewProjectile(source, position.X, position.Y, velocity.X, velocity.Y, Mod.Find<ModProjectile>("LuxorsGiftMagic").Type, (int)((double)damage * 0.8), 0f, player.whoAmI, 0f, 0f);
					}
					else if (item.CountsAsClass(DamageClass.Summon) && player.ownedProjectileCounts[Mod.Find<ModProjectile>("LuxorsGiftSummon").Type] < 1)
					{
						Projectile.NewProjectile(source, position.X, position.Y, 0f, 0f, Mod.Find<ModProjectile>("LuxorsGiftSummon").Type, damage, 0f, player.whoAmI, 0f, 0f);
					}
				}
			}
			if (player.GetModPlayer<CalamityPlayerPreTrailer>().eArtifact && item.CountsAsClass(DamageClass.Ranged) && !rogue)
			{
				velocity.X *= 1.25f;
				velocity.Y *= 1.25f;
			}
			if (player.GetModPlayer<CalamityPlayerPreTrailer>().bloodflareMage) //0 - 99
			{
				if (item.CountsAsClass(DamageClass.Magic) && Main.rand.Next(0, 100) >= 95)
				{
					if (player.whoAmI == Main.myPlayer)
					{
						Projectile.NewProjectile(source, position.X, position.Y, velocity.X, velocity.Y, Mod.Find<ModProjectile>("GhostlyBolt").Type, (int)((double)damage * (player.GetModPlayer<CalamityPlayerPreTrailer>().auricSet ? 4.2 : 2.6)), 1f, player.whoAmI, 0f, 0f);
					}
				}
			}
			if (player.GetModPlayer<CalamityPlayerPreTrailer>().bloodflareRanged) //0 - 99
			{
				if (item.CountsAsClass(DamageClass.Ranged) && !rogue && Main.rand.Next(0, 100) >= 98)
				{
					if (player.whoAmI == Main.myPlayer)
					{
						Projectile.NewProjectile(source, position.X, position.Y, velocity.X, velocity.Y, Mod.Find<ModProjectile>("BloodBomb").Type, (int)((double)damage * (player.GetModPlayer<CalamityPlayerPreTrailer>().auricSet ? 2.2 : 1.6)), 2f, player.whoAmI, 0f, 0f);
					}
				}
			}
			if (player.GetModPlayer<CalamityPlayerPreTrailer>().tarraMage)
			{
				if (player.GetModPlayer<CalamityPlayerPreTrailer>().tarraCrits >= 5 && player.whoAmI == Main.myPlayer)
				{
					player.GetModPlayer<CalamityPlayerPreTrailer>().tarraCrits = 0;
					int num106 = 9 + Main.rand.Next(3);
					for (int num107 = 0; num107 < num106; num107++)
					{
						float num110 = 0.025f * (float)num107;
						float hardar = velocity.X + (float)Main.rand.Next(-25, 26) * num110;
						float hordor = velocity.Y + (float)Main.rand.Next(-25, 26) * num110;
						float num84 = (float)Math.Sqrt((double)(velocity.X * velocity.X + velocity.Y * velocity.Y));
						num84 = item.shootSpeed / num84;
						hardar *= num84;
						hordor *= num84;
						Projectile.NewProjectile(source, position.X, position.Y, hardar, hordor, 206, (int)((double)damage * 0.2), knockback, player.whoAmI, 0.0f, 0.0f);
					}
				}
			}
			if (player.GetModPlayer<CalamityPlayerPreTrailer>().ataxiaBolt)
			{
				if (item.CountsAsClass(DamageClass.Ranged) && !rogue && Main.rand.Next(2) == 0)
				{
					if (player.whoAmI == Main.myPlayer)
					{
						Projectile.NewProjectile(source, position.X, position.Y, velocity.X * 1.25f, velocity.Y * 1.25f, Mod.Find<ModProjectile>("ChaosFlare").Type, (int)((double)damage * 0.25), 2f, player.whoAmI, 0f, 0f);
					}
				}
			}
			if (player.GetModPlayer<CalamityPlayerPreTrailer>().godSlayerRanged) //0 - 99
			{
				if (item.CountsAsClass(DamageClass.Ranged) && !rogue && Main.rand.Next(0, 100) >= 95)
				{
					if (player.whoAmI == Main.myPlayer)
					{
						Projectile.NewProjectile(source, position.X, position.Y, velocity.X * 1.25f, velocity.Y * 1.25f, Mod.Find<ModProjectile>("GodSlayerShrapnelRound").Type, (int)((double)damage * (player.GetModPlayer<CalamityPlayerPreTrailer>().auricSet ? 3.2 : 2.1)), 2f, player.whoAmI, 0f, 0f);
					}
				}
			}
			if (player.GetModPlayer<CalamityPlayerPreTrailer>().ataxiaVolley)
			{
				if (rogue && Main.rand.Next(10) == 0)
				{
					if (player.whoAmI == Main.myPlayer)
					{
						...
						for (i = 0; i < 4; i++)
						{
							...
							Projectile.NewProjectile(source, vector2.X, vector2.Y, (float)(Math.Sin(offsetAngle) * 5f), (float)(Math.Cos(offsetAngle) * 5f), Mod.Find<ModProjectile>("ChaosFlare2").Type, (int)((double)damage * 0.5), 1.25f, player.whoAmI, 0f, 0f);
							Projectile.NewProjectile(source, vector2.X, vector2.Y, (float)(-Math.Sin(offsetAngle) * 5f), (float)(-Math.Cos(offsetAngle) * 5f), Mod.Find<ModProjectile>("ChaosFlare2").Type, (int)((double)damage * 0.5), 1.25f, player.whoAmI, 0f, 0f);
						}
					}
				}
			}
			if (player.GetModPlayer<CalamityPlayerPreTrailer>().reaverDoubleTap) //0 - 99
			{
				if (item.CountsAsClass(DamageClass.Ranged) && !rogue && Main.rand.Next(0, 100) >= 90)
				{
					if (player.whoAmI == Main.myPlayer)
					{
						Projectile.NewProjectile(source, position.X, position.Y, velocity.X * 1.25f, velocity.Y * 1.25f, Mod.Find<ModProjectile>("MiniRocket").Type, (int)((double)damage * 1.3), 2f, player.whoAmI, 0f, 0f);
					}
				}
			}
			if (player.GetModPlayer<CalamityPlayerPreTrailer>().victideSet)
			{
				if ((item.CountsAsClass(DamageClass.Ranged) || item.CountsAsClass(DamageClass.Melee) || item.CountsAsClass(DamageClass.Magic) ||
					rogue || item.CountsAsClass(DamageClass.Summon)) && item.rare < 8 && Main.rand.Next(10) == 0)
				{
					if (player.whoAmI == Main.myPlayer)
					{
						Projectile.NewProjectile(source, position.X, position.Y, velocity.X * 1.25f, velocity.Y * 1.25f, Mod.Find<ModProjectile>("Seashell").Type, damage * 2, 1f, player.whoAmI, 0f, 0f);
					}
				}
			}
			return true;
		...
		public override bool OnPickup(Item item, Player player)
		{
			...
			{
				bool boostedHeart = player.GetModPlayer<CalamityPlayerPreTrailer>().photosynthesis;
			...
			if (tt2 != null)
			{
				switch (postMoonLordRarity)
				{
					...
					case 17: //Legendary Weapons
						if (item.type == Mod.Find<ModItem>("Malachite").Type)
							tt2.OverrideColor = new Color(Main.DiscoR, 203, 103);
						if (item.type == Mod.Find<ModItem>("AegisBlade").Type || item.type == Mod.Find<ModItem>("YharimsCrystal").Type)
							tt2.OverrideColor = new Color(255, Main.DiscoG, 53);
						if (item.type == Mod.Find<ModItem>("BrinyBaron").Type)
							tt2.OverrideColor = new Color(53, Main.DiscoG, 255);
						if (item.type == Mod.Find<ModItem>("CosmicDischarge").Type)
							tt2.OverrideColor = new Color(150, Main.DiscoG, 255);
						if (item.type == Mod.Find<ModItem>("BlossomFlux").Type)
							tt2.OverrideColor = new Color(Main.DiscoR, 203, 103);
						if (item.type == Mod.Find<ModItem>("SHPC").Type)
							tt2.OverrideColor = new Color(255, Main.DiscoG, 155);
						if (item.type == Mod.Find<ModItem>("Vesuvius").Type)
