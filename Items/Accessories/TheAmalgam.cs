    ...
    public class TheAmalgam : ModItem
    {
        ...
        public override void UpdateAccessory(Player player, bool hideVisual)
        {
            ...
            if (player.whoAmI == Main.myPlayer)
            {
                ...
                {
                    Projectile.NewProjectile(player.GetSource_FromThis(null), player.Center.X, player.Center.Y, 0f, -1f, Mod.Find<ModProjectile>("FungalClump").Type, (int)(250f * player.GetDamage(DamageClass.Summon).Multiplicative), 1f, Main.myPlayer, 0f, 0f);
                }
            }
            ...
            if (((double)player.velocity.X > 0 || (double)player.velocity.Y > 0 || (double)player.velocity.X < -0.1 || (double)player.velocity.Y < -0.1))
            {
                if (player.whoAmI == Main.myPlayer)
                {
                    Projectile.NewProjectile(player.GetSource_FromThis(null) ,player.Center.X, player.Center.Y, 0f, 0f, Mod.Find<ModProjectile>("PoisonousSeawater").Type, 500, 5f, player.whoAmI, 0f, 0f);
                }
            }
            if (player.immune)
            {
