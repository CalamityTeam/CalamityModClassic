	...
	public class XerocWings : ModItem
	{
		public override void SetStaticDefaults()
		{
			...
			/* Tooltip.SetDefault("Highly radioactive\n" +
				...
				"Good vertical speed\n" +
				"Flight time: 180"); */
			ArmorIDs.Wing.Sets.Stats[Item.wingSlot] = new WingStats(180, 8.5f, 2f);
		}
		...
		public override void UpdateAccessory(Player player, bool hideVisual)
		{
			if (player.controlJump && player.wingTime > 0f && !VanillaExtraJump.CloudInABottle.CanStart(player) && player.jump == 0 && player.velocity.Y != 0f && !hideVisual)
			{
				...
				int num60 = Dust.NewDust(new Vector2(player.position.X + (float)(player.width / 2) + (float)num59, player.position.Y + (float)(player.height / 2) - 15f), 30, 30, 242, 0f, 0f, 100, default(Color), 2.4f);
