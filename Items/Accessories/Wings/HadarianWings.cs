...
namespace CalamityModClassicPreTrailer.Items.Accessories.Wings
{
	...
	public class HadarianWings : ModItem
	{
		public override void SetStaticDefaults()
		{
			...
			/* Tooltip.SetDefault("Powered by the Astral Infection itself\n" +
				"Horizontal speed: 9\n" +
				"Acceleration multiplier: 1.75\n" +
				"Good vertical speed\n" +
				"Flight time: 60"); */
			ArmorIDs.Wing.Sets.Stats[Item.wingSlot] = new WingStats(60, 9f, 1.75f);
		}
		...
		public override void UpdateAccessory(Player player, bool hideVisual)
		{
			if (player.controlJump && player.wingTime > 0f && !VanillaExtraJump.CloudInABottle.CanStart(player) && player.jump == 0 && player.velocity.Y != 0f && !hideVisual)
			{
				...
				int num60 = Dust.NewDust(new Vector2(player.position.X + (float)(player.width / 2) + (float)num59, player.position.Y + (float)(player.height / 2) - 15f), 30, 30, ModContent.DustType<AstralOrange>(), 0f, 0f, 100, default(Color), 1.75f);
