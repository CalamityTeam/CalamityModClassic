	...
	public class BrimstoneFury : ModItem
	{
		...
		public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
		{
			...
			for (int i = 0; i < numProj + 1; i++)
			{
				...
				Projectile.NewProjectile(source, position.X, position.Y, perturbedSpeed.X, perturbedSpeed.Y, Mod.Find<ModProjectile>("BrimstoneBolt").Type, damage, knockback, player.whoAmI, 0f, 0f);
			}
			return false;
		}
