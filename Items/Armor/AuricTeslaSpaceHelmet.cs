...
namespace CalamityModClassicPreTrailer.Items.Armor
{
    ...
    public class AuricTeslaSpaceHelmet : ModItem
    {
        ...
        public override void UpdateArmorSet(Player player)
        {
            ...
            {
                if (player.FindBuffIndex(Mod.Find<ModBuff>("SilvaCrystal").Type) == -1)
                {
                    player.AddBuff(Mod.Find<ModBuff>("SilvaCrystal").Type, 3600, true);
                }
                if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("SilvaCrystal").Type] < 1)
                {
                    Projectile.NewProjectile(Entity.GetSource_FromThis(null), player.Center.X, player.Center.Y, 0f, -1f, Mod.Find<ModProjectile>("SilvaCrystal").Type, (int)(3000f * player.GetDamage(DamageClass.Summon).Multiplicative), 0f, Main.myPlayer, 0f, 0f);
                }
                if (player.FindBuffIndex(Mod.Find<ModBuff>("Mechworm").Type) == -1)
                {
                    player.AddBuff(Mod.Find<ModBuff>("Mechworm").Type, 3600, true);
                }
                if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("MechwormHead").Type] < 1)
                {
                    int owner = player.whoAmI;
                    int typeHead = Mod.Find<ModProjectile>("MechwormHead").Type;
                    int typeBody = Mod.Find<ModProjectile>("MechwormBody").Type;
                    int typeBody2 = Mod.Find<ModProjectile>("MechwormBody2").Type;
                    ...
                    int damage = (int)(35 * ((player.GetDamage(DamageClass.Summon).Multiplicative * 5 / 3) + ((player.GetDamage(DamageClass.Summon).Multiplicative * 0.46f) * (maxMinionScale - 1))));
        ...
        public override void AddRecipes()
        {
            ...
            recipe.AddIngredient(null, "SilvaHelmet");
            recipe.AddIngredient(null, "GodSlayerHornedHelm");
            recipe.AddIngredient(null, "BloodflareHelmet");
            recipe.AddIngredient(null, "TarragonHornedHelm");
