...
namespace CalamityModClassicPreTrailer.UI
{
    public static class AstralArcanumUI
    {
        ...
        {
            CircleTextures = ModContent.Request<Texture2D>("CalamityModClassicPreTrailer/ExtraTextures/UI/AstralArcanumCircles", AssetRequestMode.ImmediateLoad).Value;
        ...
        public static void UpdateAndDraw(SpriteBatch sb)
        {
            ...
            bool forceOpenAndTeleport = CalamityWorldPreTrailer.bossRushStage == 12 && !NPCs.CalamityGlobalNPC.AnyBossNPCS() && !Main.player[Main.myPlayer].ZoneUnderworldHeight;
            if (forceOpenAndTeleport)
            {
                Open = true;
            }
            else if (CalamityWorldPreTrailer.bossRushActive) Open = false;
            if (!Open) return;
            ...
            Utils.DrawBorderStringFourWay(sb, FontAssets.MouseText.Value, text, CenterPoint.X - size.X / 2f, CenterPoint.Y + CircleOffset + CircleTextureSize / 2 + 4, Color.White, Color.Black, default(Vector2));
        }
        ...
        public static void DoTeleportation(int circle)
        {
            ...
            {
                Main.player[Main.myPlayer].GetModPlayer<CalamityPlayerPreTrailer>().HandleTeleport(circle, false, 0);
            }
        }
