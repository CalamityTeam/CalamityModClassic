...
namespace CalamityModClassicPreTrailer.NPCs.AbyssNPCs
{
	public class GulperEelHead : ModNPC
	{
        ...
        public override void SetStaticDefaults()
		{
            ...
            {
                Scale = 0.75f,
                CustomTexturePath = "CalamityModClassicPreTrailer/NPCs/AbyssNPCs/GulperEel_Bestiary",
		...
		{
            if ((Main.player[NPC.target].Center - NPC.Center).Length() < ((Main.player[NPC.target].GetModPlayer<CalamityPlayerPreTrailer>().anechoicPlating ||
                Main.player[NPC.target].GetModPlayer<CalamityPlayerPreTrailer>().anechoicCoating) ? 100f : 600f) *
                ...
                if (!TailSpawned && NPC.ai[0] == 0f)
                {
                    ...
                    for (int num36 = 0; num36 < maxLength; num36++)
                    {
                        ...
                        if (num36 >= 0 && num36 < minLength)
                        {
                            if (num36 == 0)
                            {
                                lol = NPC.NewNPC(NPC.GetSource_FromThis(null), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), Mod.Find<ModNPC>("GulperEelBody").Type, NPC.whoAmI);
                            }
                            else
                            {
                                lol = NPC.NewNPC(NPC.GetSource_FromThis(null), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), Mod.Find<ModNPC>("GulperEelBodyAlt").Type, NPC.whoAmI);
                            }
                        }
                        else
                        {
                            lol = NPC.NewNPC(NPC.GetSource_FromThis(null), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), Mod.Find<ModNPC>("GulperEelTail").Type, NPC.whoAmI);
                        }
            ...
            if (Vector2.Distance(Main.player[NPC.target].Center, NPC.Center) > 5600f || !NPC.AnyNPCs(Mod.Find<ModNPC>("GulperEelTail").Type))
            {
                NPC.active = false;
            }
            float num188 = speed;
        ...
        public override void PostDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            vector -= new Vector2((float)ModContent.Request<Texture2D>("CalamityModClassicPreTrailer/NPCs/AbyssNPCs/GulperEelHeadGlow").Value.Width, (float)(ModContent.Request<Texture2D>("CalamityModClassicPreTrailer/NPCs/AbyssNPCs/GulperEelHeadGlow").Value.Height / Main.npcFrameCount[NPC.type])) * 1f / 2f;
            ...
            Main.spriteBatch.Draw(ModContent.Request<Texture2D>("CalamityModClassicPreTrailer/NPCs/AbyssNPCs/GulperEelHeadGlow").Value, vector,
                new Microsoft.Xna.Framework.Rectangle?(NPC.frame), color, NPC.rotation, vector11, 1f, spriteEffects, 0f);
        }

        public override float SpawnChance(NPCSpawnInfo spawnInfo)
        {
            if (spawnInfo.Player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneAbyssLayer3 && spawnInfo.Water && !NPC.AnyNPCs(Mod.Find<ModNPC>("GulperEelHead").Type))
            {
				return SpawnCondition.CaveJellyfish.Chance * 0.3f;
			}
            if (spawnInfo.Player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneAbyssLayer4 && spawnInfo.Water && !NPC.AnyNPCs(Mod.Find<ModNPC>("GulperEelHead").Type))
            {
            ...
            if (NPC.life <= 0)
            {
                if (Main.netMode != NetmodeID.Server)
                {
                    ...
                    Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
                        Mod.Find<ModGore>("GulperEel").Type, 1f);
                }
            }
        }
