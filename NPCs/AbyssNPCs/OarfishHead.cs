...
namespace CalamityModClassicPreTrailer.NPCs.AbyssNPCs
{
	public class OarfishHead : ModNPC
	{
		...
		public override void SetStaticDefaults()
		{
			...
			{
				CustomTexturePath = "CalamityModClassicPreTrailer/NPCs/AbyssNPCs/Oarfish_Bestiary",
		...
		public override void AI()
		{
			...
			if (Main.netMode != 1)
			{
				if (!TailSpawned && NPC.ai[0] == 0f)
				{
					...
					for (int num36 = 0; num36 < maxLength; num36++)
					{
						...
						{
							lol = NPC.NewNPC(NPC.GetSource_FromThis(null), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), Mod.Find<ModNPC>("OarfishBody").Type, NPC.whoAmI);
						}
						else
						{
							lol = NPC.NewNPC(NPC.GetSource_FromThis(null), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), Mod.Find<ModNPC>("OarfishTail").Type, NPC.whoAmI);
						}
			...
			if (Vector2.Distance(Main.player[NPC.target].Center, NPC.Center) > 5600f || !NPC.AnyNPCs(Mod.Find<ModNPC>("OarfishTail").Type))
			{
				NPC.active = false;
			}
			float num188 = speed;
			...
			if (num42 > 0)
			{
				...
				if ((Main.player[NPC.target].Center - NPC.Center).Length() > ((Main.player[NPC.target].GetModPlayer<CalamityPlayerPreTrailer>().anechoicPlating ||
					Main.player[NPC.target].GetModPlayer<CalamityPlayerPreTrailer>().anechoicCoating) ? 150f : 250f) *
		...
		{
			if (spawnInfo.Player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneAbyssLayer2 && spawnInfo.Water && !NPC.AnyNPCs(Mod.Find<ModNPC>("OarfishHead").Type))
			{
				return SpawnCondition.CaveJellyfish.Chance * 0.3f;
			}
			if (spawnInfo.Player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneAbyssLayer3 && spawnInfo.Water && !NPC.AnyNPCs(Mod.Find<ModNPC>("OarfishHead").Type))
			{
		...
		public override void OnHitPlayer(Player target, Player.HurtInfo hurtInfo)
		{
			...
			{
				target.AddBuff(Mod.Find<ModBuff>("MarkedforDeath").Type, 90);
				target.AddBuff(Mod.Find<ModBuff>("Horror").Type, 90, true);
			}
		}
	}
}