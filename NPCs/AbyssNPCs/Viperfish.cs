...
namespace CalamityModClassicPreTrailer.NPCs.AbyssNPCs
{
	public class Viperfish : ModNPC
	{
		...
		public override void SetDefaults()
		{
			...
			Banner = NPC.type;
			BannerItem = Mod.Find<ModItem>("ViperfishBanner").Type;
		...
		public override void AI()
		{
			...
			if (NPC.wet)
			{
				...
				if (Main.player[NPC.target].wet && !Main.player[NPC.target].dead &&
					...
					Main.player[NPC.target].GetModPlayer<CalamityPlayerPreTrailer>().anechoicCoating) ? 200f : 600f) *
		...
		public override void PostDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
		{
			...
			vector -= new Vector2((float)ModContent.Request<Texture2D>("CalamityModClassicPreTrailer/NPCs/AbyssNPCs/ViperfishGlow").Value.Width, (float)(ModContent.Request<Texture2D>("CalamityModClassicPreTrailer/NPCs/AbyssNPCs/ViperfishGlow").Value.Height / Main.npcFrameCount[NPC.type])) * 1f / 2f;
			...
			Main.spriteBatch.Draw(ModContent.Request<Texture2D>("CalamityModClassicPreTrailer/NPCs/AbyssNPCs/ViperfishGlow").Value, vector,
		...
		public override void ModifyNPCLoot(NPCLoot npcLoot)
		{
			...
			fakeCalorPlantDead.OnSuccess(new CommonDrop(Mod.Find<ModItem>("Lumenite").Type, 4));
			...
			if (NPC.life <= 0)
			{
				if (Main.netMode != NetmodeID.Server)
				{
					...
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
						Mod.Find<ModGore>("ViperFish").Type, 1f);
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
						Mod.Find<ModGore>("ViperFish2").Type, 1f);
				}
			}
		}
	}
