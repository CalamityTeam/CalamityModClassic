...
namespace CalamityModClassicPreTrailer.NPCs.AbyssNPCs
{
	public class ColossalSquid : ModNPC
	{
		...
		public override void SetDefaults()
		{
			...
			NPC.rarity = 2;
			Banner = NPC.type;
			BannerItem = Mod.Find<ModItem>("ColossalSquidBanner").Type;
		...
		public override void AI()
		{
			...
			else
			{
				...
				if ((Main.player[NPC.target].wet && !Main.player[NPC.target].dead &&
					...
					Main.player[NPC.target].GetModPlayer<CalamityPlayerPreTrailer>().anechoicCoating) ? 150f : 300f) *
				...
				if (hasBeenHit)
				{
					...
					if (Main.netMode != 1 && NPC.localAI[0] >= 150f)
					{
						...
						Projectile.NewProjectile(NPC.GetSource_FromThis(null), NPC.Center.X, NPC.Center.Y + 60, 0f, 2f, Mod.Find<ModProjectile>("InkBomb").Type, damage, 0f, Main.myPlayer, 0f, 0f);
					}
					NPC.rotation = NPC.velocity.X * 0.05f;
					NPC.velocity *= 0.975f;
					float num263 = 2.5f;
		...
		{
			if (spawnInfo.Player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneAbyssLayer3 && spawnInfo.Water && !NPC.AnyNPCs(Mod.Find<ModNPC>("ColossalSquid").Type) &&
				!NPC.AnyNPCs(Mod.Find<ModNPC>("Reaper").Type) && !NPC.AnyNPCs(Mod.Find<ModNPC>("EidolonWyrmHead").Type) && !NPC.AnyNPCs(Mod.Find<ModNPC>("EidolonWyrmHeadHuge").Type))
			{
		...
		{
			target.AddBuff(Mod.Find<ModBuff>("CrushDepth").Type, 600, true);
		...
		{
			npcLoot.Add(new CommonDrop(ItemID.BlackInk, 1, 3, 6));
			...
			if (NPC.life <= 0)
			{
				if (Main.netMode != NetmodeID.Server)
				{
					...
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
						Mod.Find<ModGore>("ColossalSquid").Type, 1f);
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
						Mod.Find<ModGore>("ColossalSquid2").Type, 1f);
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
						Mod.Find<ModGore>("ColossalSquid3").Type, 1f);
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
						Mod.Find<ModGore>("ColossalSquid4").Type, 1f);
				}
			}
		}
	}
