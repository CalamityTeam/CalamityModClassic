...
namespace CalamityModClassicPreTrailer.NPCs.AbyssNPCs
{
	public class AquaticUrchin : ModNPC
	{
		...
		public override void SetDefaults()
		{
			...
			Banner = NPC.type;
			BannerItem = Mod.Find<ModItem>("AquaticUrchinBanner").Type;
		...
		public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
		{
			...
			{
				new FlavorTextBestiaryInfoElement("Brethren of their ocean counterparts, these Urchins' spines are coated with the venom that surrounds them, making them deadly on touch.")
			});
		}
        ...
        public override float SpawnChance(NPCSpawnInfo spawnInfo)
        {
            ...
            if (spawnInfo.Player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneSulphur && spawnInfo.Water && NPC.CountNPCS(Mod.Find<ModNPC>("AquaticUrchin").Type) < 2)
            {
                return 0.2f;
            }
            return 0f;
			...
			if (NPC.life <= 0)
			{
				if (Main.netMode != NetmodeID.Server)
				{
					...
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
						Mod.Find<ModGore>("AquaticUrchin").Type, 1f);
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
						Mod.Find<ModGore>("AquaticUrchin2").Type, 1f);
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
						Mod.Find<ModGore>("AquaticUrchin3").Type, 1f);
				}
			}
		}
	}
