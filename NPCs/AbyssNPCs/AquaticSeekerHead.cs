	...
	public class AquaticSeekerHead : ModNPC
	{
		...
		public override void SetStaticDefaults()
		{
			...
			NPCID.Sets.NPCBestiaryDrawModifiers value = new NPCID.Sets.NPCBestiaryDrawModifiers()
			{
				Scale = 0.7f,
				PortraitScale = 0.7f,
				CustomTexturePath = "CalamityModClassicPreTrailer/NPCs/AbyssNPCs/AquaticSeeker_Bestiary",
		...
		public override void AI()
		{
			...
			if (Main.netMode != 1)
			{
				if (!TailSpawned && NPC.ai[0] == 0f)
				{
					...
					for (int num36 = 0; num36 < maxLength; num36++)
					{
						...
						{
							lol = NPC.NewNPC(NPC.GetSource_FromThis(null), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), Mod.Find<ModNPC>("AquaticSeekerBody").Type, NPC.whoAmI);
						}
						else
						{
							lol = NPC.NewNPC(NPC.GetSource_FromThis(null), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), Mod.Find<ModNPC>("AquaticSeekerTail").Type, NPC.whoAmI);
						}
			...
			if (Vector2.Distance(Main.player[NPC.target].Center, NPC.Center) > 5600f || !NPC.AnyNPCs(Mod.Find<ModNPC>("AquaticSeekerTail").Type))
			{
				NPC.active = false;
			}
			float num188 = speed;
		...
		public override float SpawnChance(NPCSpawnInfo spawnInfo)
		{
			...
			{
				if (!NPC.AnyNPCs(Mod.Find<ModNPC>("AquaticSeekerHead").Type))
					return 0.01f;
			}
			return 0f;
		}
			...
			if (NPC.life <= 0)
			{
				if (Main.netMode != NetmodeID.Server)
				{
					...
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
						Mod.Find<ModGore>("AquaticSeekerHead").Type, 1f);
				}
			}
		}
