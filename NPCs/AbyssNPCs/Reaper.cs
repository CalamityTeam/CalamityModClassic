...
namespace CalamityModClassicPreTrailer.NPCs.AbyssNPCs
{
	public class Reaper : ModNPC
	{
		...
		public override void SetDefaults()
		{
			...
			NPC.rarity = 2;
			Banner = NPC.type;
			BannerItem = Mod.Find<ModItem>("ReaperSharkBanner").Type;
		...
		public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
		{
			...
			{
				new FlavorTextBestiaryInfoElement("do you wanna be suplexed into oblivion? because it won't ask twice.")
			});
		}
		...
		public override void AI()
		{
			...
			if (NPC.soundDelay <= 0)
			{
				...
				if (hasBeenHit)
				{
					SoundEngine.PlaySound(new SoundStyle("CalamityModClassicPreTrailer/Sounds/Custom/ReaperEnragedRoar"), NPC.position);
				}
				else
				{
					SoundEngine.PlaySound(new SoundStyle("CalamityModClassicPreTrailer/Sounds/Custom/ReaperSearchRoar"), NPC.position);
				}
			}
			if (phase3 || phase1)
			{
		...
		{
			if (spawnInfo.Player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneAbyssLayer3 && spawnInfo.Water && !NPC.AnyNPCs(Mod.Find<ModNPC>("Reaper").Type) &&
				!NPC.AnyNPCs(Mod.Find<ModNPC>("ColossalSquid").Type) && !NPC.AnyNPCs(Mod.Find<ModNPC>("EidolonWyrmHead").Type) && !NPC.AnyNPCs(Mod.Find<ModNPC>("EidolonWyrmHeadHuge").Type))
			{
				return SpawnCondition.CaveJellyfish.Chance * 0.6f;
			}
			if (spawnInfo.Player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneAbyssLayer4 && spawnInfo.Water && !NPC.AnyNPCs(Mod.Find<ModNPC>("Reaper").Type))
			{
		...
		public override void ModifyNPCLoot(NPCLoot npcLoot)
		{
			...
			npcLoot.Add(new CommonDrop(Mod.Find<ModItem>("DepthCells").Type, 1, 40, 51));
