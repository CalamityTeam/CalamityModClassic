...
namespace CalamityModClassicPreTrailer.NPCs.AbyssNPCs
{
	public class AquaticScourgeBody : ModNPC
	{
		...
		public override void AI()
		{
            ...
            {
                int num = (Main.expertMode || CalamityWorldPreTrailer.bossRushActive) ? 4 : 3;
                ...
                if (NPC.localAI[0] >= (float)Main.rand.Next(700, 10000))
                {
                    ...
                    if (Collision.CanHit(NPC.position, NPC.width, NPC.height, Main.player[NPC.target].position, Main.player[NPC.target].width, Main.player[NPC.target].height))
                    {
                        ...
                        int projectileType = Mod.Find<ModProjectile>("SandTooth").Type;
                        int damage = (Main.expertMode || CalamityWorldPreTrailer.bossRushActive) ? 26 : 37;
                        float num941 = 5f;
            ...
            if (NPC.life <= 0)
            {
                if (Main.netMode != NetmodeID.Server)
                {
                    ...
                    Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
                        Mod.Find<ModGore>("ASBody").Type, 1f);
                    Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
                        Mod.Find<ModGore>("ASBody2").Type, 1f);
                    Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
                        Mod.Find<ModGore>("ASBody3").Type, 1f);
                    Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
                        Mod.Find<ModGore>("ASBody4").Type, 1f);
                }
            }
        }
