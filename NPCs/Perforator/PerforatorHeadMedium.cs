...
namespace CalamityModClassicPreTrailer.NPCs.Perforator
{
	...
	public class PerforatorHeadMedium : ModNPC
	{
        ...
        private int minLength = (CalamityWorldPreTrailer.death || CalamityWorldPreTrailer.bossRushActive) ? 5 : 10;
        private int maxLength = (CalamityWorldPreTrailer.death || CalamityWorldPreTrailer.bossRushActive) ? 6 : 11;
		...
		public override void SetStaticDefaults()
		{
			...
			NPCID.Sets.NPCBestiaryDrawModifiers value = new NPCID.Sets.NPCBestiaryDrawModifiers(0)
			{
				Scale = 0.7f,
				PortraitScale = 0.7f,
				CustomTexturePath = "CalamityModClassicPreTrailer/NPCs/Perforator/PerforatorMedium_Bestiary",
		...
		public override void AI()
		{
			...
			if (!TailSpawned)
            {
                ...
                for (int num36 = 0; num36 < maxLength; num36++)
                {
                    ...
                    {
                        lol = NPC.NewNPC(NPC.GetSource_FromThis(null), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), Mod.Find<ModNPC>("PerforatorBodyMedium").Type, NPC.whoAmI);
                    }
                    else
                    {
                        lol = NPC.NewNPC(NPC.GetSource_FromThis(null), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), Mod.Find<ModNPC>("PerforatorTailMedium").Type, NPC.whoAmI);
                    }
			...
			if (NPC.life <= 0)
			{
				...
				if (Main.netMode != NetmodeID.Server)
				{
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
						Mod.Find<ModGore>("MediumPerf").Type, 1f);
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
		...
		{
			npcLoot.Add(new CommonDrop(Mod.Find<ModItem>("BloodSample").Type, 1, 3, 8));
			npcLoot.Add(new CommonDrop(ItemID.Vertebrae, 1, 2, 5));
