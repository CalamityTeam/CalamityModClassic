...
namespace CalamityModClassic1Point1.NPCs.Perforator
{
	...
	public class PerforatorHeadMedium : ModNPC
	{
        ...
        public override void SetStaticDefaults()
        {
            ...
            {
                CustomTexturePath = "CalamityModClassic1Point1/NPCs/Perforator/Bestiary_Medium"
            };
        ...
        public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
        {
            bestiaryEntry.Info.AddRange(new IBestiaryInfoElement[]
            {
                ...
                new FlavorTextBestiaryInfoElement("The middest of the bloodworms.")
        ...
        public override float SpawnChance(NPCSpawnInfo spawnInfo)
		{
            ...
            return spawnInfo.Player.ZoneCrimson && Main.hardMode ? 0.00001f : 0f; 
		}
		
		public override void AI()
		{
			bool isBlood = Main.player[NPC.target].ZoneCrimson;
			if (NPC.ai[3] > 0f)
			{
			...
			{
				speed = isBlood ? 20f : 25f;
				turnSpeed = isBlood ? 0.18f : 0.2f;
			}
			else if (NPC.life < (NPC.lifeMax * 0.6f) && NPC.life >= (NPC.lifeMax * 0.4f))
			{
				speed = isBlood ? 22f : 28f;
				turnSpeed = isBlood ? 0.2f : 0.23f;
			}
			else if (NPC.life < (NPC.lifeMax * 0.4f) && NPC.life >= (NPC.lifeMax * 0.2f))
			{
				speed = isBlood ? 25f : 32f;
				turnSpeed = isBlood ? 0.23f : 0.27f;
			}
			else if (NPC.life < (NPC.lifeMax * 0.2f))
			{
				speed = isBlood ? 29f : 37f;
				turnSpeed = isBlood ? 0.27f : 0.32f;
			}
			NPC.alpha -= 42;
			if (NPC.alpha < 0)
			{
			...
			if (!TailSpawned)
            {
                ...
                for (int num36 = 0; num36 < 31; num36++)
                {
                    ...
                    if (num36 >= 0 && num36 < 30)
                    {
                        lol = NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), Mod.Find<ModNPC>("PerforatorBodyMedium").Type, NPC.whoAmI);
                    }
                    else
                    {
                        lol = NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), Mod.Find<ModNPC>("PerforatorTailMedium").Type, NPC.whoAmI);
                    }
        ...
        {
            npcLoot.Add(ItemDropRule.ByCondition(new Conditions.NotExpert(), ModContent.ItemType<BloodSample>(), 1, 5, 10));
            npcLoot.Add(ItemDropRule.ByCondition(new Conditions.NotExpert(), ItemID.Vertebrae, 1, 2, 7));
            npcLoot.Add(ItemDropRule.ByCondition(new Conditions.NotExpert(), ItemID.CrimtaneBar, 1, 2, 3));
        }
		...
		{
			target.AddBuff(Mod.Find<ModBuff>("BurningBlood").Type, 200, true);
			if (Main.expertMode)
			{
