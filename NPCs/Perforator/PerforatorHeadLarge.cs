...
namespace CalamityModClassicPreTrailer.NPCs.Perforator
{
	...
	public class PerforatorHeadLarge : ModNPC
	{
        ...
        private int minLength = (CalamityWorldPreTrailer.death || CalamityWorldPreTrailer.bossRushActive) ? 7 : 15;
        private int maxLength = (CalamityWorldPreTrailer.death || CalamityWorldPreTrailer.bossRushActive) ? 8 : 16;
		...
		public override void SetStaticDefaults()
		{
			...
			NPCID.Sets.NPCBestiaryDrawModifiers value = new NPCID.Sets.NPCBestiaryDrawModifiers(0)
			{
				Scale = 0.7f,
				PortraitScale = 0.7f,
				CustomTexturePath = "CalamityModClassicPreTrailer/NPCs/Perforator/PerforatorLarge_Bestiary",
		...
		public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
		{
			bestiaryEntry.Info.AddRange(new IBestiaryInfoElement[]
			{
				...
				new FlavorTextBestiaryInfoElement("The largest of the perforators, its purpose is to devour.")
			});
		}
		...
		public override void AI()
		{
			...
			if (!TailSpawned)
            {
                ...
                for (int num36 = 0; num36 < maxLength; num36++)
                {
                    ...
                    {
                        lol = NPC.NewNPC(NPC.GetSource_FromThis(null), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), Mod.Find<ModNPC>("PerforatorBodyLarge").Type, NPC.whoAmI);
                    }
                    else
                    {
                        lol = NPC.NewNPC(NPC.GetSource_FromThis(null), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), Mod.Find<ModNPC>("PerforatorTailLarge").Type, NPC.whoAmI);
                    }
                    if (num36 % 2 == 0)
                    {
			...
			if (NPC.life <= 0)
			{
				...
				if (Main.netMode != NetmodeID.Server)
				{
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
						Mod.Find<ModGore>("LargePerf").Type, 1f);
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
						Mod.Find<ModGore>("LargePerf2").Type, 1f);
				}
			}
		}
		...
		{
			npcLoot.Add(new CommonDrop(Mod.Find<ModItem>("BloodSample").Type, 1, 4, 9));
			npcLoot.Add(new CommonDrop(ItemID.Vertebrae, 1, 3, 6));
			npcLoot.Add(new CommonDrop(ItemID.CrimtaneBar, 1, 2, 5));
		}
		...
		{
			target.AddBuff(Mod.Find<ModBuff>("BurningBlood").Type, 300, true);
