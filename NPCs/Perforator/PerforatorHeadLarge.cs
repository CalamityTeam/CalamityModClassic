...
namespace CalamityModClassic1Point1.NPCs.Perforator
{
	...
	public class PerforatorHeadLarge : ModNPC
	{
        ...
        public override void SetStaticDefaults()
        {
            ...
            {
                CustomTexturePath = "CalamityModClassic1Point1/NPCs/Perforator/Bestiary_Large"
            };
            NPCID.Sets.NPCBestiaryDrawOffset[Type] = value;
        }
        public override void SetDefaults()
		{
			//NPC.name = "PerforatorHeadLarge");
        ...
        public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
        {
            bestiaryEntry.Info.AddRange(new IBestiaryInfoElement[]
            {
                ...
                new FlavorTextBestiaryInfoElement("The biggest of the bloodworms. It's the one that matters the most.")
        ...
        public override void AI()
		{
			...
			{
				speed = isBlood ? 18f : 23f;
				turnSpeed = isBlood ? 0.16f : 0.18f;
			}
			else if (NPC.life < (NPC.lifeMax * 0.6f) && NPC.life >= (NPC.lifeMax * 0.4f))
			{
				speed = isBlood ? 20f : 26f;
				turnSpeed = isBlood ? 0.18f : 0.21f;
			}
			else if (NPC.life < (NPC.lifeMax * 0.4f) && NPC.life >= (NPC.lifeMax * 0.2f))
			{
				speed = isBlood ? 23f : 30f;
				turnSpeed = isBlood ? 0.21f : 0.25f;
			}
			else if (NPC.life < (NPC.lifeMax * 0.2f))
			{
				speed = isBlood ? 27f : 35f;
				turnSpeed = isBlood ? 0.25f : 0.3f;
			}
			NPC.alpha -= 42;
			if (NPC.alpha < 0)
			{
			...
			if (!TailSpawned)
            {
                ...
                for (int num36 = 0; num36 < 41; num36++)
                {
                    int lol = 0;
                    if (num36 >= 0 && num36 < 40)
                    {
