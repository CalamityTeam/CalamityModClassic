...
namespace CalamityModClassicPreTrailer.NPCs.AstrumDeus
{
	...
	public class AstrumDeusHeadSpectral : ModNPC
	{
		...
		public override void SetDefaults()
		{
			...
			NPC.height = 56; //216
			NPC.defense = 40;
            NPC.lifeMax = CalamityWorldPreTrailer.revenge ? 35850 : 25000; //250000
			...
			{
				NPC.lifeMax = CalamityWorldPreTrailer.death ? 1500000 : 1300000;
			}
		...
		{
			colorChange = NPC.AnyNPCs(Mod.Find<ModNPC>("AstrumDeusHead").Type);
            ...
            float speedLimit = CalamityWorldPreTrailer.revenge ? 9f : 7f;
            float turnSpeedLimit = CalamityWorldPreTrailer.revenge ? 0.14f : 0.12f;
            if (CalamityWorldPreTrailer.death || CalamityWorldPreTrailer.bossRushActive)
            {
                speedLimit = ((NPC.GetGlobalNPC<CalamityGlobalNPC>().enraged || (Config.BossRushXerocCurse && CalamityWorldPreTrailer.bossRushActive)) ? 15f : 12f);
                turnSpeedLimit = ((NPC.GetGlobalNPC<CalamityGlobalNPC>().enraged || (Config.BossRushXerocCurse && CalamityWorldPreTrailer.bossRushActive)) ? 0.2f : 0.16f);
            }
			...
			if (Main.netMode != 1)
			{
				if (!TailSpawned && NPC.ai[0] == 0f)
	            {
	                ...
	                for (int num36 = 0; num36 < maxLength; num36++)
	                {
	                    ...
	                    {
	                        lol = NPC.NewNPC(NPC.GetSource_FromThis(null), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), Mod.Find<ModNPC>("AstrumDeusBodySpectral").Type, NPC.whoAmI);
	                    }
	                    else
	                    {
	                        lol = NPC.NewNPC(NPC.GetSource_FromThis(null), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), Mod.Find<ModNPC>("AstrumDeusTailSpectral").Type, NPC.whoAmI);
	                    }
                        if (num36 % 2 == 0)
                        {
		...
		{
			potionType = Mod.Find<ModItem>("Stardust").Type;
		}
