	...
	public class AstrumDeusBodySpectral : ModNPC
	{
		...
		public override void AI()
		{
			...
			if (Main.netMode != 1)
			{
				...
				{
					shootTime += CalamityWorldPreTrailer.death ? 4 : 2;
                }
				if ((double)NPC.life <= (double)NPC.lifeMax * 0.3 || CalamityWorldPreTrailer.bossRushActive || Main.player[NPC.target].gravDir == -1f)
				{
					shootTime += CalamityWorldPreTrailer.death ? 6 : 2;
				}
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            Texture2D texture = ModContent.Request<Texture2D>("CalamityModClassicPreTrailer/NPCs/AstrumDeus/AstrumDeusBodyAltSpectral").Value;
			...
			Texture2D texture2D3 = (NPC.localAI[3] == 1f ? texture : TextureAssets.Npc[NPC.type].Value);
			...
			CalamityModClassicPreTrailer.DrawTexture(spriteBatch, texture2D3, 0, NPC, newColor);
            return false;
        }
		...
		{
            if (NPC.CountNPCS(Mod.Find<ModNPC>("AstrumDeusProbe3").Type) < 6 && CalamityWorldPreTrailer.revenge)
            {
