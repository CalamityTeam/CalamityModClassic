...
namespace CalamityModClassicPreTrailer.NPCs.StormWeaver
{
	public class StormWeaverBodyNaked : ModNPC
	{
        ...
        public override void HitEffect(NPC.HitInfo hit)
        {
            if (NPC.life <= 0)
            {
	            if (Main.netMode != NetmodeID.Server)
	            {
		            Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
			            Mod.Find<ModGore>("SWNude2").Type, 1f);
		            Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
			            Mod.Find<ModGore>("SWNude3").Type, 1f);
		            Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
			            Mod.Find<ModGore>("SWNude4").Type, 1f);
	            }
            ...
            if (NPC.CountNPCS(Mod.Find<ModNPC>("StasisProbeNaked").Type) < 3)
            {
                spawn--;
                if (NPC.life > 0 && Main.netMode != 1 && spawn <= 0)
                {
                    spawn = 14;
                    int num660 = NPC.NewNPC(NPC.GetSource_FromThis(null), (int)(NPC.position.X + (float)(NPC.width / 2)), (int)(NPC.position.Y + (float)NPC.height), Mod.Find<ModNPC>("StasisProbeNaked").Type, 0, 0f, 0f, 0f, 0f, 255);
                    if (Main.netMode == 2)
                    {
