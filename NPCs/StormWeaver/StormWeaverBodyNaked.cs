...
namespace CalamityModClassic1Point2.NPCs.StormWeaver
{
	public class StormWeaverBodyNaked : ModNPC
	{
		...
		{
			if (NPC.CountNPCS(Mod.Find<ModNPC>("StasisProbeNaked").Type) < 10)
			{
				spawn--;
				if (NPC.life > 0 && Main.netMode != NetmodeID.MultiplayerClient && spawn <= 0)
				{
					spawn = 14;
					int num660 = NPC.NewNPC(NPC.GetSource_FromThis(), (int)(NPC.position.X + (float)(NPC.width / 2)), (int)(NPC.position.Y + (float)NPC.height), Mod.Find<ModNPC>("StasisProbeNaked").Type, 0, 0f, 0f, 0f, 0f, 255);
