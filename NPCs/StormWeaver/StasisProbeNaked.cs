	...
	public class StasisProbeNaked : ModNPC
	{
		...
		public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
		{
			bestiaryEntry.Info.AddRange(new IBestiaryInfoElement[]
			{
				...
				new FlavorTextBestiaryInfoElement("One would assume it being a probe would mean it's a machine but nope!")
			});
		}
		...
		public override void AI()
		{
            ...
            if (Main.netMode != 1 && NPC.localAI[0] >= 90f)
			{
				...
				if (Collision.CanHit(NPC.position, NPC.width, NPC.height, Main.player[NPC.target].position, Main.player[NPC.target].width, Main.player[NPC.target].height))
				{
					...
					int num9 = 84;
					int beam = Projectile.NewProjectile(Entity.GetSource_FromThis(null), vector.X, vector.Y, num4 * 0.75f, num5 * 0.75f, num9, num8, 0f, Main.myPlayer, 0f, 0f);
