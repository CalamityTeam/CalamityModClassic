	...
	public class DankCreeper : ModNPC
	{
		...
		public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
		{
			bestiaryEntry.Info.AddRange(new IBestiaryInfoElement[]
			{
				...
				new FlavorTextBestiaryInfoElement("Servants summoned by the Hive Mind to serve as guardians for it.")
			});
		}
		...
		public override void HitEffect(NPC.HitInfo hit)
		{
			...
			if (NPC.life <= 0)
			{
				if (Main.netMode != NetmodeID.Server)
				{
					for (int k = 0; k < 20; k++)
					{
						Dust.NewDust(NPC.position, NPC.width, NPC.height, 13, hit.HitDirection, -1f, 0, default(Color),
							1f);
					}

					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
						Mod.Find<ModGore>("DankCreeperGore").Type, 1f);
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
						Mod.Find<ModGore>("DankCreeperGore2").Type, 1f);
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
						Mod.Find<ModGore>("DankCreeperGore3").Type, 1f);
				}
			}
		}
		...
		public override void OnKill()
		{
			...
			{
				Projectile.NewProjectile(NPC.GetSource_FromThis(null), NPC.Center.X, NPC.Center.Y, 0f, 0f, Mod.Find<ModProjectile>("ShadeNimbusHostile").Type, 14, 0f, Main.myPlayer, 0f, 0f);
			}
		}
	}
}