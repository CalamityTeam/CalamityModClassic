...
namespace CalamityModClassicPreTrailer.NPCs.TownNPCs
{
	...
	public class SEAHOE : ModNPC
	{
		...
		public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
		{
			...
			{
				new FlavorTextBestiaryInfoElement("A King displaced from his realm; He knows his tragedy, but has come to peace with it.")
			});
		}

		public override bool CanTownNPCSpawn(int numTownNPCs)/* tModPorter Suggestion: Copy the implementation of NPC.SpawnAllowed_Merchant in vanilla if you to count money, and be sure to set a flag when unlocked, so you don't count every tick. */
		{
			return CalamityWorldPreTrailer.downedCLAM && CalamityWorldPreTrailer.downedDesertScourge;
		}

		public override List<string> SetNPCNameList()/* tModPorter Suggestion: Return a list of names */
		{
			return new List<string>() {"Amidias"};
		}
		...
		public override string GetChat()
		{
			...
			int cirrus = NPC.FindFirstNPC(Mod.Find<ModNPC>("FAP").Type);
			if (cirrus != -1)
			...
			if (Main.player[Main.myPlayer].GetModPlayer<CalamityPlayerPreTrailer>().sirenPet)
			{
		...
		public string Lore()
		{
			...
			else if (CalamityWorldPreTrailer.downedDoG)
			{
			...
			else if (CalamityWorldPreTrailer.downedProvidence)
			{
			...
			else if (CalamityWorldPreTrailer.downedCryogen)
			{
		...
		public override void OnChatButtonClicked(bool firstButton, ref string shopName)
		{
			...
			else
			{
				...
				Player player = Main.player[Main.myPlayer];
				player.AddBuff(Mod.Find<ModBuff>("AmidiasBlessing").Type, 36000);
			}
		}
		...
		public override void AddShops()
		{
			...
			shop.Add(ModContent.ItemType<Shellshooter>())
				...
				.AddWithCustomValue(ItemID.TruffleWorm, Item.buyPrice(0, 40, 0, 0), Condition.DownedDukeFishron)
				.Register();
		}
			...
			if (NPC.life <= 0)
			{
				if (Main.netMode != NetmodeID.Server)
				{
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
						Mod.Find<ModGore>("Amidias").Type, 1f);
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
						Mod.Find<ModGore>("Amidias2").Type, 1f);
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
						Mod.Find<ModGore>("Amidias3").Type, 1f);
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position, NPC.velocity,
						Mod.Find<ModGore>("Amidias4").Type, 1f);
				}
			}
		}
		...
		{
			projType = Mod.Find<ModProjectile>("SnapClamProj").Type;
			attackDelay = 1;
		}
