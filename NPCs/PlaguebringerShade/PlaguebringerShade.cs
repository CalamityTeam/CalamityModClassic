...
namespace CalamityModClassic1Point1.NPCs.PlaguebringerShade
{
	public class PlaguebringerShade : ModNPC
	{
		public override void SetDefaults()
		{
			//NPC.name = "PlaguebringerShade");
			//Tooltip.SetDefault("Plaguebringer Shade");
			NPC.damage = 90; //150
			NPC.npcSlots = 1f;
			NPC.width = 66; //324
			NPC.height = 66; //216
			NPC.defense = 30;
			NPC.lifeMax = 7500; //250000
        ...
        public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
        {
            bestiaryEntry.Info.AddRange(new IBestiaryInfoElement[]
            {
                ...
                new FlavorTextBestiaryInfoElement("The biological terror of the Jungle is pretty decent at making holograms, and they hurt!")
        ...
        public override void AI()
		{
			...
			else if (NPC.ai[0] == 2f)
			{
				NPC.TargetClosest(true);
				NPC.spriteDirection = NPC.direction;
				float num1053 = 20f; //12
				float num1054 = 0.175f; //0.07
			...
			{
				float num1065 = 10f; //changed from 4
				float num1066 = 0.125f; //changed from 0.05
				if (NPC.downedMoonlord)
				{
					num1066 = 0.135f;
					num1065 = 11f;
				...
				if (flag104 && NPC.position.Y + (float)NPC.height < Main.player[NPC.target].position.Y && Collision.CanHit(vector121, 1, 1, Main.player[NPC.target].position, Main.player[NPC.target].width, Main.player[NPC.target].height))
				{
					...
					if (Main.netMode != 1)
					{
						float num1070 = 8.5f; //changed from 8
						num1070 += 2.25f; //changed from 2
						if (NPC.downedMoonlord)
						{
							num1070 += 1f;
						}
		...
		public override void OnHitPlayer(Player target, Player.HurtInfo hurtInfo)
		{
			if (Main.expertMode)
			{
				target.AddBuff(Mod.Find<ModBuff>("Plague").Type, 160, true);
			}
			else
			{
				target.AddBuff(Mod.Find<ModBuff>("Plague").Type, 160, true);
