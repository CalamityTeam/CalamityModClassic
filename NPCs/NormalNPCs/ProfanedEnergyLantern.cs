				...
				if (!Main.player[NPC.target].dead)
				{
					...
					if (NPC.localAI[1] >= 600f)
					{
						if (!Collision.SolidCollision(NPC.position, NPC.width, NPC.height))
						{
							...
							Projectile.NewProjectile(NPC.GetSource_FromThis(null), NPC.Center.X, NPC.Center.Y, num189, num190, num9, 40, 0f, Main.myPlayer, 0f, 0f);
							NPC.localAI[1] = 0f;
							return;
						}
						NPC.localAI[1] = 250f;
            ...
            if (CalamityGlobalNPC.energyFlame > 0)
            {
                ...
                while (draw)
                {
                    ...
                    else
                    {
                        ...
                        Main.spriteBatch.Draw(ModContent.Request<Texture2D>("CalamityModClassicPreTrailer/NPCs/NormalNPCs/ProfanedEnergySegment").Value, new Vector2(center.X - Main.screenPosition.X, center.Y - Main.screenPosition.Y),
                            new Microsoft.Xna.Framework.Rectangle?(new Microsoft.Xna.Framework.Rectangle(0, 0, ModContent.Request<Texture2D>("CalamityModClassicPreTrailer/NPCs/NormalNPCs/ProfanedEnergySegment").Value.Width, ModContent.Request<Texture2D>("CalamityModClassicPreTrailer/NPCs/NormalNPCs/ProfanedEnergySegment").Value.Height)), color, rotation,
                            new Vector2((float)ModContent.Request<Texture2D>("CalamityModClassicPreTrailer/NPCs/NormalNPCs/ProfanedEnergySegment").Value.Width * 0.5f, (float)ModContent.Request<Texture2D>("CalamityModClassicPreTrailer/NPCs/NormalNPCs/ProfanedEnergySegment").Value.Height * 0.5f), 1f, SpriteEffects.None, 0f);
                    }
                }
            }
			return true;
