...
namespace CalamityModClassic1Point1.NPCs.TheDevourerofGods
{
	...
	public class DevourerofGodsHead : ModNPC
	{
		...
		public int minLength = 120;
		public int maxLength = 121;
        ...
        public override void SetStaticDefaults()
        {
            NPCID.Sets.NPCBestiaryDrawModifiers value = new NPCID.Sets.NPCBestiaryDrawModifiers(0)
            {
                Scale = 0.8f,
                PortraitScale = 0.8f,
                CustomTexturePath = "CalamityModClassic1Point1/NPCs/TheDevourerofGods/Bestiary",
        ...
        public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
        {
            bestiaryEntry.Info.AddRange(new IBestiaryInfoElement[]
            {
                ...
                new FlavorTextBestiaryInfoElement("A relatively decently constructed cosmic serpent. It's quite faulty, but holds potential for the future.")
        ...
        public override void AI()
		{
			...
			if ((NPC.life <= NPC.lifeMax * 0.5f))
			{
				...
				if (halfLife == 0)
				{
					Main.NewText("Don't get cocky, kid!", Color.Cyan.R, Color.Cyan.G, Color.Cyan.B);
					halfLife++;
				}
			}
			...
			else if (Main.expertMode && NPC.life < (NPC.lifeMax * 0.1f))
			{
				speed = 36f;
				turnSpeed = 0.56f;
			}
			...
			if (Main.netMode != 1)
            {
	            if (!tail && NPC.ai[0] == 0f)
				{
					...
					for (int num36 = 0; num36 < maxLength; num36++)
	                {
	                    ...
	                    {
	                        lol = NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), Mod.Find<ModNPC>("DevourerofGodsBody").Type, NPC.whoAmI);
	                    }
	                    else
	                    {
	                        lol = NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.position.X + (NPC.width / 2), (int)NPC.position.Y + (NPC.height / 2), Mod.Find<ModNPC>("DevourerofGodsTail").Type, NPC.whoAmI);
	                    }
				...
				if (beamPortal >= 1800f)
				{
					...
					if (Collision.CanHit(NPC.position, NPC.width, NPC.height, Main.player[NPC.target].position, Main.player[NPC.target].width, Main.player[NPC.target].height))
					{
						...
						Main.projectile[num947].timeLeft = 900;
						NPC.netUpdate = true;
					}
				}
            }
			...
			else
			{
				...
				{
					NPC.TargetClosest(true);
					NPC.velocity.Y = NPC.velocity.Y + (turnSpeed * 0.35f);
					...
					else if (NPC.velocity.Y > 4f)
					{
						if (NPC.velocity.X < 0f)
						{
							NPC.velocity.X = NPC.velocity.X + num189 * 0.9f; //changed from 0.9
						}
						else
						{
							NPC.velocity.X = NPC.velocity.X - num189 * 0.9f; //changed from 0.9
        ...
        public override void ModifyNPCLoot(NPCLoot npcLoot)
        {
            ...
            npcLoot.Add(ItemDropRule.ByCondition(new Conditions.NotExpert(), ModContent.ItemType<DeathhailStaff>(), 12));
            npcLoot.Add(ItemDropRule.ByCondition(new Conditions.NotExpert(), ModContent.ItemType<Eradicator>(), 12));
            npcLoot.Add(ItemDropRule.ByCondition(new Conditions.NotExpert(), ModContent.ItemType<Excelsus>(), 12));
            npcLoot.Add(ItemDropRule.ByCondition(new Conditions.NotExpert(), ModContent.ItemType<Items.Weapons.TheObliterator>(), 12));
            npcLoot.Add(ItemDropRule.ByCondition(new Conditions.NotExpert(), ModContent.ItemType<EradicatorMelee>(), 12));
            npcLoot.Add(ItemDropRule.ByCondition(new Conditions.NotExpert(), ModContent.ItemType<Deathwind>(), 12));
            npcLoot.Add(ItemDropRule.ByCondition(new Conditions.NotExpert(), ModContent.ItemType<StaffoftheMechworm>(), 12));
        }
		...
		public override void OnHitPlayer(Player target, Player.HurtInfo hurtInfo)
		{
			...
			if (num == 0)
			{
				Main.NewText("A fatal mistake!", Color.Cyan.R, Color.Cyan.G, Color.Cyan.B);
			}
			else if (num == 1)
			{
				Main.NewText("Good luck recovering from that!", Color.Cyan.R, Color.Cyan.G, Color.Cyan.B);
			}
			else if (num == 2)
			{
				Main.NewText("Delicious...", Color.Cyan.R, Color.Cyan.G, Color.Cyan.B);
			}
			else if (num == 3)
			{
				Main.NewText("Did that hurt?", Color.Cyan.R, Color.Cyan.G, Color.Cyan.B);
			}
			else if (num == 4)
			{
				Main.NewText("Nothing personal, kid", Color.Cyan.R, Color.Cyan.G, Color.Cyan.B);
			}
			if (Main.expertMode)
			{
