...
namespace CalamityModClassic1Point1.NPCs.SlimeSpawnCrimson2
{
	public class SlimeSpawnCrimson2 : ModNPC
	{
		...
		public override void SetDefaults()
		{
			...
			//Tooltip.SetDefault(" Spiked Crimson Slime Spawn");
			NPC.aiStyle = 1;
			NPC.damage = 60;
			NPC.width = 40; //324
			NPC.height = 30; //216
        ...
        public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
        {
            bestiaryEntry.Info.AddRange(new IBestiaryInfoElement[]
            {
                ...
                new FlavorTextBestiaryInfoElement("Spike.")
			...
			if (!NPC.wet && !Main.player[NPC.target].npcTypeNoAggro[NPC.type])
			{
				...
				if (Main.expertMode && num16 < 120f && Collision.CanHit(NPC.position, NPC.width, NPC.height, Main.player[NPC.target].position, Main.player[NPC.target].width, Main.player[NPC.target].height) && NPC.velocity.Y == 0f)
				{
					...
					if (Main.netMode != 1 && spikeTimer == 0f)
					{
						for (int n = 0; n < 5; n++)
						{
							...
							Projectile.NewProjectile(NPC.GetSource_FromThis(), vector3.X, vector3.Y, vector4.X, vector4.Y, Mod.Find<ModProjectile>("CrimsonSpike").Type, 9, 0f, Main.myPlayer, 0f, 0f);
							spikeTimer = 30f;
						}
					}
				}
				else if (num16 < 200f && Collision.CanHit(NPC.position, NPC.width, NPC.height, Main.player[NPC.target].position, Main.player[NPC.target].width, Main.player[NPC.target].height) && NPC.velocity.Y == 0f)
				{
					...
					if (Main.netMode != 1 && spikeTimer == 0f)
					{
						...
						num15 *= num16;
						spikeTimer = 50f;
						Projectile.NewProjectile(NPC.GetSource_FromThis(), vector3.X, vector3.Y, num14, num15, Mod.Find<ModProjectile>("CrimsonSpike").Type, 9, 0f, Main.myPlayer, 0f, 0f);
					}
				}
			}
		}
