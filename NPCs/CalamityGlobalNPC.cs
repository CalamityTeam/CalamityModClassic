...
namespace CalamityModClassicPreTrailer.NPCs
{
	public class CalamityGlobalNPC : GlobalNPC
	{
		...
		public override void UpdateLifeRegen(NPC npc, ref int damage)
		{
			...
			if (hurtByAbyss)
			{
				npc.AddBuff(Mod.Find<ModBuff>("CrushDepth").Type, 2);
			...
			bool hurtByPiss = (npc.wet && npc.damage > 0 && !npc.boss && !npc.friendly && !npc.dontTakeDamage && CalamityWorldPreTrailer.sulphurTiles > 30 &&
				!npc.buffImmune[BuffID.Poisoned] && !npc.buffImmune[Mod.Find<ModBuff>("CrushDepth").Type]);
			if (hurtByPiss)
			{
				npc.AddBuff(BuffID.Poisoned, 2);
			}
			if (Main.raining && npc.damage > 0 && !npc.boss &&
				!npc.friendly && !npc.dontTakeDamage && CalamityWorldPreTrailer.sulphurTiles > 30 &&
				!npc.buffImmune[BuffID.Poisoned] && !npc.buffImmune[Mod.Find<ModBuff>("CrushDepth").Type])
			{
			...
			if (npc.venom)
			{
				...
				for (int j = 0; j < 1000; j++)
				{
					if (Main.projectile[j].active &&
						(Main.projectile[j].type == Mod.Find<ModProjectile>("Lionfish").Type || Main.projectile[j].type == Mod.Find<ModProjectile>("SulphuricAcidCannon2").Type) &&
			...
			if (shellfishVore)
			{
				...
				{
					if (Main.projectile[j].active &&
						(Main.projectile[j].type == Mod.Find<ModProjectile>("Shellfish").Type) &&
						Main.projectile[j].ai[0] == 1f && Main.projectile[j].ai[1] == (float)npc.whoAmI &&
						amount < 5)
					{
						amount++;
			...
			if (clamDebuff)
			{
				...
				{
					if (Main.projectile[j].active &&
						(Main.projectile[j].type == Mod.Find<ModProjectile>("SnapClamProj").Type) &&
						Main.projectile[j].ai[0] == 1f && Main.projectile[j].ai[1] == (float)npc.whoAmI &&
						amount < 2)
					{
						amount++;
			...
			if (eFreeze && !CalamityWorldPreTrailer.bossRushActive)
			{
				npc.velocity.X = 0f;
		...
		public override void SetDefaults(NPC npc)
		{
			...
			if (CalamityWorldPreTrailer.bossRushActive)
			{
				if (!npc.friendly)
				{
					...
					if (npc.type != Mod.Find<ModNPC>("DevourerofGodsHeadS").Type && npc.type != Mod.Find<ModNPC>("DevourerofGodsBodyS").Type && npc.type != Mod.Find<ModNPC>("DevourerofGodsTailS").Type)
					{
						npc.buffImmune[BuffID.Ichor] = false;
						npc.buffImmune[BuffID.CursedInferno] = false;
					}
					npc.buffImmune[Mod.Find<ModBuff>("Enraged").Type] = false;
				}
			}
			else
			{
				if (CalamityModClassicPreTrailer.enemyImmunityList.Contains(npc.type))
				{
				...
				if (npc.buffImmune[Mod.Find<ModBuff>("Enraged").Type])
				{
					npc.buffImmune[Mod.Find<ModBuff>("Enraged").Type] = false;
				}
			}
			...
			if (npc.type == NPCID.CultistBoss)
			{
				...
				{
					npc.lifeMax = (int)((double)npc.lifeMax * (CalamityWorldPreTrailer.revenge ? 1.55 : 1.15));
				}
				npc.npcSlots = 20f;
			}
			...
			if (CalamityWorldPreTrailer.revenge)
			{
				...
				else if (npc.type == NPCID.MoonLordCore) //final boss in hardmode, gets a big boost in stats
				{
					if (CalamityWorldPreTrailer.death)
					{
				...
				else if (npc.type >= 454 && npc.type <= 459)
				{
					if (CalamityWorldPreTrailer.death)
					{
				...
				else if (npc.type == NPCID.WallofFlesh || npc.type == NPCID.WallofFleshEye) //final boss prehm, gets a big boost in stats
				{
				...
				if (npc.type == Mod.Find<ModNPC>("Cnidrion").Type || npc.type == Mod.Find<ModNPC>("DesertScourgeBody").Type || npc.type == Mod.Find<ModNPC>("ColossalSquid").Type ||
					npc.type == Mod.Find<ModNPC>("Siren").Type || npc.type == Mod.Find<ModNPC>("ThiccWaifu").Type || npc.type == Mod.Find<ModNPC>("ProfanedGuardianBoss3").Type ||
					npc.type == Mod.Find<ModNPC>("ScornEater").Type || npc.type == Mod.Find<ModNPC>("AquaticScourgeBody").Type || npc.type == Mod.Find<ModNPC>("AquaticScourgeBodyAlt").Type ||
					npc.type == Mod.Find<ModNPC>("Mauler").Type || npc.type == Mod.Find<ModNPC>("EutrophicRay").Type)
				{
					protection = 0.05f;
				}
				else if (npc.type == Mod.Find<ModNPC>("AstrumDeusBody").Type || npc.type == Mod.Find<ModNPC>("SoulSeeker").Type || npc.type == Mod.Find<ModNPC>("DesertScourgeTail").Type ||
					npc.type == Mod.Find<ModNPC>("Horse").Type || npc.type == Mod.Find<ModNPC>("ProfanedEnergyBody").Type || npc.type == Mod.Find<ModNPC>("ScavengerClawLeft").Type ||
					npc.type == Mod.Find<ModNPC>("ScavengerClawRight").Type || npc.type == Mod.Find<ModNPC>("ScavengerHead").Type || npc.type == Mod.Find<ModNPC>("MantisShrimp").Type ||
					npc.type == Mod.Find<ModNPC>("PhantomDebris").Type || npc.type == Mod.Find<ModNPC>("AstrumDeusHead").Type || npc.type == Mod.Find<ModNPC>("AquaticScourgeHead").Type ||
					npc.type == Mod.Find<ModNPC>("Cryon").Type || npc.type == Mod.Find<ModNPC>("Cryogen").Type)
				{
					protection = 0.1f;
				}
				else if (npc.type == Mod.Find<ModNPC>("ArmoredDiggerHead").Type || npc.type == Mod.Find<ModNPC>("AstralProbe").Type || npc.type == Mod.Find<ModNPC>("Calamitas").Type ||
					npc.type == Mod.Find<ModNPC>("CalamitasRun").Type || npc.type == Mod.Find<ModNPC>("CalamitasRun2").Type || npc.type == Mod.Find<ModNPC>("CalamitasRun3").Type ||
					npc.type == Mod.Find<ModNPC>("SoulSlurper").Type || npc.type == Mod.Find<ModNPC>("ProvSpawnHealer").Type || npc.type == Mod.Find<ModNPC>("Gnasher").Type ||
					npc.type == Mod.Find<ModNPC>("ScavengerLegLeft").Type || npc.type == Mod.Find<ModNPC>("ScavengerLegRight").Type || npc.type == Mod.Find<ModNPC>("ShockstormShuttle").Type ||
					npc.type == Mod.Find<ModNPC>("Reaper").Type || npc.type == Mod.Find<ModNPC>("OverloadedSoldier").Type || npc.type == Mod.Find<ModNPC>("AquaticScourgeTail").Type ||
					npc.type == Mod.Find<ModNPC>("EidolonWyrmHead").Type || npc.type == Mod.Find<ModNPC>("Aries").Type || npc.type == Mod.Find<ModNPC>("AstralachneaGround").Type ||
					npc.type == Mod.Find<ModNPC>("AstralachneaWall").Type || npc.type == Mod.Find<ModNPC>("Astrageldon").Type || npc.type == Mod.Find<ModNPC>("Atlas").Type ||
					npc.type == Mod.Find<ModNPC>("BigSightseer").Type || npc.type == Mod.Find<ModNPC>("FusionFeeder").Type || npc.type == Mod.Find<ModNPC>("Hadarian").Type ||
					npc.type == Mod.Find<ModNPC>("Hive").Type || npc.type == Mod.Find<ModNPC>("Mantis").Type || npc.type == Mod.Find<ModNPC>("Nova").Type || npc.type == Mod.Find<ModNPC>("SmallSightseer").Type ||
					npc.type == Mod.Find<ModNPC>("StellarCulex").Type)
				{
					protection = 0.15f;
				}
				else if (npc.type == Mod.Find<ModNPC>("AstrumDeusProbe3").Type || npc.type == Mod.Find<ModNPC>("PlaguebringerShade").Type || npc.type == Mod.Find<ModNPC>("BlindedAngler").Type)
				{
					protection = 0.2f;
				}
				else if (npc.type == Mod.Find<ModNPC>("PlaguebringerGoliath").Type || npc.type == Mod.Find<ModNPC>("ProfanedGuardianBoss2").Type || npc.type == Mod.Find<ModNPC>("SandTortoise").Type ||
					npc.type == Mod.Find<ModNPC>("StasisProbe").Type || npc.type == Mod.Find<ModNPC>("BobbitWormHead").Type || npc.type == Mod.Find<ModNPC>("GreatSandShark").Type ||
					npc.type == Mod.Find<ModNPC>("Clam").Type || npc.type == Mod.Find<ModNPC>("PrismTurtle").Type)
				{
					protection = 0.25f;
				}
				else if (npc.type == Mod.Find<ModNPC>("ProvSpawnOffense").Type || npc.type == Mod.Find<ModNPC>("GiantClam").Type)
				{
					protection = 0.3f;
				}
				else if (npc.type == Mod.Find<ModNPC>("ArmoredDiggerBody").Type || npc.type == Mod.Find<ModNPC>("AstrumDeusTail").Type || npc.type == Mod.Find<ModNPC>("DespairStone").Type ||
					npc.type == Mod.Find<ModNPC>("SoulSeekerSupreme").Type || npc.type == Mod.Find<ModNPC>("Leviathan").Type)
				{
					protection = 0.35f;
				}
				else if (npc.type == Mod.Find<ModNPC>("CryogenIce").Type || npc.type == Mod.Find<ModNPC>("ProfanedGuardianBoss").Type || npc.type == Mod.Find<ModNPC>("ProvSpawnDefense").Type ||
					npc.type == Mod.Find<ModNPC>("ScavengerBody").Type)
				{
					protection = 0.4f;
				}
				else if (npc.type == Mod.Find<ModNPC>("ArmoredDiggerTail").Type)
				{
					protection = 0.45f;
				}
				else if (npc.type == Mod.Find<ModNPC>("SirenIce").Type)
				{
					protection = 0.5f;
				}
				else
			...
			if (npc.type == Mod.Find<ModNPC>("SCalWormBody").Type || npc.type == Mod.Find<ModNPC>("SCalWormHead").Type || npc.type == Mod.Find<ModNPC>("SCalWormBodyWeak").Type ||
					npc.type == Mod.Find<ModNPC>("SCalWormTail").Type || npc.type == Mod.Find<ModNPC>("EidolonWyrmHeadHuge").Type)
			{
				protection = 0.999999f;
			}
			if (protection > 0f)
			...
			{
				if (CalamityModClassicPreTrailer.pumpkinMoonBuffList.Contains(npc.type))
				{
				...
				else if (CalamityModClassicPreTrailer.frostMoonBuffList.Contains(npc.type))
				{
			...
			if (CalamityModClassicPreTrailer.eclipseBuffList.Contains(npc.type) && CalamityWorldPreTrailer.buffedEclipse)
			{
			...
			{
				if (CalamityModClassicPreTrailer.dungeonEnemyBuffList.Contains(npc.type))
				{
			...
			if (CalamityWorldPreTrailer.revenge)
			{
				if (CalamityModClassicPreTrailer.revengeanceEnemyBuffList.Contains(npc.type))
				{
		...
		public override void ApplyDifficultyAndPlayerScaling(NPC npc, int numPlayers, float balance, float bossAdjustment)/* tModPorter Note: bossLifeScale -> balance (bossAdjustment is different, see the docs for details) */
		{
			if (Main.netMode != 0)
			{
				if (numPlayers > 1)
				{
					if (((npc.boss || CalamityModClassicPreTrailer.bossScaleList.Contains(npc.type)) && npc.type < NPCID.Count) ||
						(npc.ModNPC != null && npc.ModNPC.Mod.Name.Equals("CalamityModClassicPreTrailer")))
					{
						double scalar = 1.0;
		...
		{
			if (Main.pumpkinMoon && CalamityWorldPreTrailer.downedDoG && !npc.boss && !npc.friendly && !npc.dontTakeDamage)
				cooldownSlot = 1;
			else if (Main.snowMoon && CalamityWorldPreTrailer.downedDoG && !npc.boss && !npc.friendly && !npc.dontTakeDamage)
				cooldownSlot = 1;
			else if (Main.eclipse && CalamityWorldPreTrailer.buffedEclipse && !npc.boss && !npc.friendly && !npc.dontTakeDamage)
				cooldownSlot = 1;
		...
		public override void ModifyHitPlayer(NPC npc, Player target, ref Player.HurtModifiers modifiers)
		{
			...
			if (target.GetModPlayer<CalamityPlayerPreTrailer>().beeResist)
			{
				if (CalamityModClassicPreTrailer.beeEnemyList.Contains(npc.type))
				{
		...
		public override bool PreAI(NPC npc)
		{
			if (setNewName)
			{
				...
				{
					case NPCID.Guide:
						switch (npc.townNpcVariationIndex) //34 guide names
						{
							case 0:
								npc.GivenName = "Lapp";
								break;
						...
						break;
					case NPCID.Wizard:
						switch (npc.townNpcVariationIndex) //23 wizard names
						{
							case 0:
					...
					case NPCID.Steampunker:
						switch (npc.townNpcVariationIndex) //21 steampunker names
						{
							case 0:
						...
						break;
					case NPCID.Stylist:
						switch (npc.townNpcVariationIndex) //20 stylist names
						{
							case 0:
								npc.GivenName = "Amber";
								break;
					...
					case NPCID.WitchDoctor:
						switch (npc.townNpcVariationIndex) //10 witch doctor names
						{
							case 0:
			...
			{
				if (Main.player[npc.target].GetModPlayer<CalamityPlayerPreTrailer>().queenBeeLore)
				{
					npc.damage = 0;
			...
			#region BossRush
			if (CalamityWorldPreTrailer.bossRushActive && !npc.friendly && !npc.townNPC)
			{
				switch (CalamityWorldPreTrailer.bossRushStage)
				{
					...
					case 5:
						...
						break;
					case 6:
						if (npc.type != Mod.Find<ModNPC>("ProfanedGuardianBoss").Type && npc.type != Mod.Find<ModNPC>("ProfanedGuardianBoss2").Type &&
							npc.type != Mod.Find<ModNPC>("ProfanedGuardianBoss3").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
					...
					case 7:
						...
						break;
					case 8:
						if (npc.type != Mod.Find<ModNPC>("Astrageldon").Type && npc.type != Mod.Find<ModNPC>("AstralSlime").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
					...
					case 10:
						...
						break;
					case 11:
						if (npc.type != Mod.Find<ModNPC>("Bumblefuck").Type && npc.type != Mod.Find<ModNPC>("Bumblefuck2").Type &&
					...
					case 12:
						...
						break;
					case 13:
						if (npc.type != Mod.Find<ModNPC>("HiveMind").Type && npc.type != Mod.Find<ModNPC>("HiveMindP2").Type &&
							npc.type != Mod.Find<ModNPC>("DarkHeart").Type && npc.type != Mod.Find<ModNPC>("HiveBlob").Type &&
							npc.type != Mod.Find<ModNPC>("DankCreeper").Type && npc.type != Mod.Find<ModNPC>("HiveBlob2").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
					...
					case 14:
						...
						break;
					case 15:
						if (npc.type != Mod.Find<ModNPC>("StormWeaverHead").Type && npc.type != Mod.Find<ModNPC>("StormWeaverBody").Type &&
							npc.type != Mod.Find<ModNPC>("StormWeaverTail").Type && npc.type != Mod.Find<ModNPC>("StormWeaverHeadNaked").Type &&
							npc.type != Mod.Find<ModNPC>("StormWeaverBodyNaked").Type && npc.type != Mod.Find<ModNPC>("StormWeaverTailNaked").Type &&
							npc.type != Mod.Find<ModNPC>("StasisProbe").Type && npc.type != Mod.Find<ModNPC>("StasisProbeNaked").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
						break;
					case 16:
						if (npc.type != Mod.Find<ModNPC>("AquaticScourgeHead").Type && npc.type != Mod.Find<ModNPC>("AquaticScourgeBody").Type &&
							npc.type != Mod.Find<ModNPC>("AquaticScourgeBodyAlt").Type && npc.type != Mod.Find<ModNPC>("AquaticScourgeTail").Type &&
							npc.type != Mod.Find<ModNPC>("AquaticParasite").Type && npc.type != Mod.Find<ModNPC>("AquaticUrchin").Type &&
							npc.type != Mod.Find<ModNPC>("AquaticSeekerHead").Type && npc.type != Mod.Find<ModNPC>("AquaticSeekerBody").Type &&
							npc.type != Mod.Find<ModNPC>("AquaticSeekerTail").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
						break;
					case 17:
						if (npc.type != Mod.Find<ModNPC>("DesertScourgeHead").Type && npc.type != Mod.Find<ModNPC>("DesertScourgeBody").Type &&
							npc.type != Mod.Find<ModNPC>("DesertScourgeTail").Type && npc.type != Mod.Find<ModNPC>("DesertScourgeHeadSmall").Type &&
							npc.type != Mod.Find<ModNPC>("DesertScourgeBodySmall").Type && npc.type != Mod.Find<ModNPC>("DesertScourgeTailSmall").Type &&
							npc.type != Mod.Find<ModNPC>("DriedSeekerHead").Type && npc.type != Mod.Find<ModNPC>("DriedSeekerBody").Type &&
							npc.type != Mod.Find<ModNPC>("DriedSeekerTail").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
					...
					case 18:
						if (npc.type != NPCID.CultistBoss && npc.type != NPCID.CultistBossClone && npc.type != NPCID.CultistDragonHead &&
							...
							npc.type != NPCID.AncientLight && npc.type != NPCID.AncientDoom && npc.type != Mod.Find<ModNPC>("Eidolist").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
						break;
					case 19:
						if (npc.type != Mod.Find<ModNPC>("CrabulonIdle").Type && npc.type != Mod.Find<ModNPC>("CrabShroom").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
					...
					case 20:
						...
						break;
					case 21:
						if (npc.type != Mod.Find<ModNPC>("CeaselessVoid").Type && npc.type != Mod.Find<ModNPC>("DarkEnergy").Type &&
							npc.type != Mod.Find<ModNPC>("DarkEnergy2").Type && npc.type != Mod.Find<ModNPC>("DarkEnergy3").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
						break;
					case 22:
						if (npc.type != Mod.Find<ModNPC>("PerforatorHive").Type && npc.type != Mod.Find<ModNPC>("PerforatorHeadLarge").Type &&
							npc.type != Mod.Find<ModNPC>("PerforatorBodyLarge").Type && npc.type != Mod.Find<ModNPC>("PerforatorTailLarge").Type &&
							npc.type != Mod.Find<ModNPC>("PerforatorHeadMedium").Type && npc.type != Mod.Find<ModNPC>("PerforatorBodyMedium").Type &&
							npc.type != Mod.Find<ModNPC>("PerforatorTailMedium").Type && npc.type != Mod.Find<ModNPC>("PerforatorHeadSmall").Type &&
							npc.type != Mod.Find<ModNPC>("PerforatorBodySmall").Type && npc.type != Mod.Find<ModNPC>("PerforatorTailSmall").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
						break;
					case 23:
						if (npc.type != Mod.Find<ModNPC>("Cryogen").Type && npc.type != Mod.Find<ModNPC>("CryogenIce").Type &&
							npc.type != Mod.Find<ModNPC>("IceMass").Type && npc.type != Mod.Find<ModNPC>("Cryocore").Type &&
							npc.type != Mod.Find<ModNPC>("Cryocore2").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
						break;
					case 24:
						if (npc.type != Mod.Find<ModNPC>("BrimstoneElemental").Type && npc.type != Mod.Find<ModNPC>("Brimling").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
						break;
					case 25:
						if (npc.type != Mod.Find<ModNPC>("CosmicWraith").Type && npc.type != Mod.Find<ModNPC>("SignusBomb").Type &&
							npc.type != Mod.Find<ModNPC>("CosmicLantern").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
						break;
					case 26:
						if (npc.type != Mod.Find<ModNPC>("ScavengerBody").Type && npc.type != Mod.Find<ModNPC>("ScavengerHead").Type &&
							npc.type != Mod.Find<ModNPC>("ScavengerClawLeft").Type && npc.type != Mod.Find<ModNPC>("ScavengerClawRight").Type &&
							npc.type != Mod.Find<ModNPC>("ScavengerLegLeft").Type && npc.type != Mod.Find<ModNPC>("ScavengerLegRight").Type &&
							npc.type != Mod.Find<ModNPC>("ScavengerHead2").Type && npc.type != Mod.Find<ModNPC>("RockPillar").Type &&
							npc.type != Mod.Find<ModNPC>("FlamePillar").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
					...
					case 28:
						if (npc.type != NPCID.MoonLordCore && npc.type != NPCID.MoonLordHead && npc.type != NPCID.MoonLordHand &&
							npc.type != NPCID.MoonLordFreeEye && npc.type != Mod.Find<ModNPC>("Eidolist").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
						break;
					case 29:
						if (npc.type != Mod.Find<ModNPC>("AstrumDeusHead").Type && npc.type != Mod.Find<ModNPC>("AstrumDeusBody").Type &&
							npc.type != Mod.Find<ModNPC>("AstrumDeusTail").Type && npc.type != Mod.Find<ModNPC>("AstrumDeusHeadSpectral").Type &&
							npc.type != Mod.Find<ModNPC>("AstrumDeusBodySpectral").Type && npc.type != Mod.Find<ModNPC>("AstrumDeusTailSpectral").Type &&
							npc.type != Mod.Find<ModNPC>("AstrumDeusProbe").Type && npc.type != Mod.Find<ModNPC>("AstrumDeusProbe2").Type &&
							npc.type != Mod.Find<ModNPC>("AstrumDeusProbe3").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
						break;
					case 30:
						if (npc.type != Mod.Find<ModNPC>("Polterghast").Type && npc.type != Mod.Find<ModNPC>("PhantomFuckYou").Type &&
							npc.type != Mod.Find<ModNPC>("PolterghastHook").Type && npc.type != Mod.Find<ModNPC>("PolterPhantom").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
						break;
					case 31:
						if (npc.type != Mod.Find<ModNPC>("PlaguebringerGoliath").Type && npc.type != Mod.Find<ModNPC>("PlagueBeeG").Type &&
							npc.type != Mod.Find<ModNPC>("PlagueBeeLargeG").Type && npc.type != Mod.Find<ModNPC>("PlagueHomingMissile").Type &&
							npc.type != Mod.Find<ModNPC>("PlagueMine").Type && npc.type != Mod.Find<ModNPC>("PlaguebringerShade").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
						break;
					case 32:
						if (npc.type != Mod.Find<ModNPC>("Calamitas").Type && npc.type != Mod.Find<ModNPC>("CalamitasRun").Type &&
							npc.type != Mod.Find<ModNPC>("CalamitasRun2").Type && npc.type != Mod.Find<ModNPC>("CalamitasRun3").Type &&
							npc.type != Mod.Find<ModNPC>("LifeSeeker").Type && npc.type != Mod.Find<ModNPC>("SoulSeeker").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
						break;
					case 33:
						if (npc.type != Mod.Find<ModNPC>("Siren").Type && npc.type != Mod.Find<ModNPC>("Leviathan").Type &&
							npc.type != Mod.Find<ModNPC>("AquaticAberration").Type && npc.type != Mod.Find<ModNPC>("Parasea").Type &&
							npc.type != Mod.Find<ModNPC>("SirenClone").Type && npc.type != Mod.Find<ModNPC>("SirenIce").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
						break;
					case 34:
						if (npc.type != Mod.Find<ModNPC>("SlimeGod").Type && npc.type != Mod.Find<ModNPC>("SlimeGodRun").Type &&
							npc.type != Mod.Find<ModNPC>("SlimeGodCore").Type && npc.type != Mod.Find<ModNPC>("SlimeGodSplit").Type &&
							npc.type != Mod.Find<ModNPC>("SlimeGodRunSplit").Type && npc.type != Mod.Find<ModNPC>("SlimeSpawnCorrupt").Type &&
							npc.type != Mod.Find<ModNPC>("SlimeSpawnCorrupt2").Type && npc.type != Mod.Find<ModNPC>("SlimeSpawnCrimson").Type &&
							npc.type != Mod.Find<ModNPC>("SlimeSpawnCrimson2").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
						break;
					case 35:
						if (npc.type != Mod.Find<ModNPC>("Providence").Type && npc.type != Mod.Find<ModNPC>("ProvSpawnDefense").Type &&
							npc.type != Mod.Find<ModNPC>("ProvSpawnOffense").Type && npc.type != Mod.Find<ModNPC>("ProvSpawnHealer").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
						break;
					case 36:
						if (npc.type != Mod.Find<ModNPC>("SupremeCalamitas").Type && npc.type != Mod.Find<ModNPC>("SCalWormBody").Type &&
							npc.type != Mod.Find<ModNPC>("SCalWormBodyWeak").Type && npc.type != Mod.Find<ModNPC>("SCalWormHead").Type &&
							npc.type != Mod.Find<ModNPC>("SCalWormTail").Type && npc.type != Mod.Find<ModNPC>("SoulSeekerSupreme").Type &&
							npc.type != Mod.Find<ModNPC>("SCalWormHeart").Type && npc.type != Mod.Find<ModNPC>("SupremeCataclysm").Type &&
							npc.type != Mod.Find<ModNPC>("SupremeCatastrophe").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
						break;
					case 37:
						if (npc.type != Mod.Find<ModNPC>("Yharon").Type && npc.type != Mod.Find<ModNPC>("DetonatingFlare").Type &&
							npc.type != Mod.Find<ModNPC>("DetonatingFlare2").Type && npc.type != Mod.Find<ModNPC>("DetonatingFlare3").Type &&
							npc.type != Mod.Find<ModNPC>("Bumblefuck3").Type && npc.type != Mod.Find<ModNPC>("Bumblefuck4").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
						}
						break;
					case 38:
						if (npc.type != Mod.Find<ModNPC>("DevourerofGodsHeadS").Type && npc.type != Mod.Find<ModNPC>("DevourerofGodsBodyS").Type &&
							npc.type != Mod.Find<ModNPC>("DevourerofGodsTailS").Type)
						{
							npc.active = false;
							npc.netUpdate = true;
				...
				if (npc.type == NPCID.KingSlime)
				{
					...
					if (!Main.player[npc.target].dead && npc.ai[2] >= 300f && npc.ai[1] < 5f && npc.velocity.Y == 0f)
					{
						...
						if (Main.netMode != 1)
						{
							...
							while (!flag10 && num238 < 100)
							{
								...
								if ((num240 < point4.Y - num237 || num240 > point4.Y + num237 || num239 < point4.X - num237 || num239 > point4.X + num237) && (num240 < point3.Y - num236 || num240 > point3.Y + num236 || num239 < point3.X - num236 || num239 > point3.X + num236) && !Main.tile[num239, num240].HasUnactuatedTile)
								{
									int num241 = num240;
									int num242 = 0;
									bool flag11 = Main.tile[num239, num241].HasUnactuatedTile && Main.tileSolid[(int)Main.tile[num239, num241].TileType] && !Main.tileSolidTop[(int)Main.tile[num239, num241].TileType];
									if (flag11)
									{
										num242 = 1;
									}
									else
									{
										while (num242 < 150 && num241 + num242 < Main.maxTilesY)
										{
											...
											bool flag12 = Main.tile[num239, num243].HasUnactuatedTile && Main.tileSolid[(int)Main.tile[num239, num243].TileType] && !Main.tileSolidTop[(int)Main.tile[num239, num243].TileType];
											if (flag12)
											{
												num242--;
												break;
					...
					if (npc.ai[1] == 5f)
					{
						...
						if (npc.ai[0] == 60f)
						{
							Gore.NewGore(npc.GetSource_FromThis(null), npc.Center + new Vector2(-40f, (float)(-(float)npc.height / 2)), npc.velocity, 734, 1f);
						}
						...
						if (Main.netMode != 1)
						{
							...
							if ((float)(npc.life + num251) < npc.ai[3])
							{
								...
								for (int num253 = 0; num253 < num252; num253 = num + 1)
								{
									...
									int num255 = NPC.NewNPC(npc.GetSource_FromThis(null), x, y, num254, 0, 0f, 0f, 0f, 0f, 255);
									Main.npc[num255].SetDefaults(num254, default);
				...
				else if (npc.type == NPCID.BrainofCthulhu)
				{
					...
					if (Main.netMode != 1 && npc.localAI[0] == 0f)
					{
						...
						for (int num789 = 0; num789 < 20; num789 = num + 1)
						{
							...
							int num792 = NPC.NewNPC(npc.GetSource_FromThis(null), (int)num790, (int)num791, 267, 0, 0f, 0f, 0f, 0f, 255);
					...
					if (npc.ai[0] < 0f)
					{
						if (npc.localAI[2] == 0f)
						{
							...
							npc.localAI[2] = 1f;
							Gore.NewGore(npc.GetSource_FromThis(null), npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), 392, 1f);
							Gore.NewGore(npc.GetSource_FromThis(null), npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), 393, 1f);
							Gore.NewGore(npc.GetSource_FromThis(null), npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), 394, 1f);
							Gore.NewGore(npc.GetSource_FromThis(null), npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), 395, 1f);
							int num;
						...
						if (newAI[0] >= 60f)
						{
							...
							{
								int creeper = NPC.NewNPC(npc.GetSource_FromThis(null), (int)(npc.position.X + (float)(npc.width / 2) + npc.velocity.X), (int)(npc.position.Y + (float)(npc.height / 2) + npc.velocity.Y), 267, 0, 0f, 0f, 0f, 0f, 255);
					...
					if (newAI[0] >= 180f)
					{
						...
						if (Collision.CanHit(npc.Center, 1, 1, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
						{
							...
							if (Main.netMode != 1)
							{
								...
								Projectile.NewProjectile(npc.GetSource_FromThis(null), vector34.X, vector34.Y, num349, num350, num420, num419, 0f, Main.myPlayer, 0f, 0f);
							}
						}
					}
				}
					...
					else if (npc.ai[0] == 1f)
					{
						...
						if (Collision.CanHit(vector76, 1, 1, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height) && flag38)
						{
							...
							if (Main.netMode != 1)
							{
								...
								int num616 = NPC.NewNPC(npc.GetSource_FromThis(null), (int)vector76.X, (int)vector76.Y, num615, 0, 0f, 0f, 0f, 0f, 255);
					...
					else if (npc.ai[0] == 3f)
					{
						...
						if (flag39 && npc.position.Y + (float)npc.height < Main.player[npc.target].position.Y && Collision.CanHit(vector78, 1, 1, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
						{
							...
							if (Main.netMode != 1)
							{
								...
								int num628 = 11;
								int num629 = 55;
								int num630 = Projectile.NewProjectile(npc.GetSource_FromThis(null), vector78.X, vector78.Y, num625, num626, num629, num628, 0f, Main.myPlayer, 0f, 0f);
				...
				else if (npc.type == NPCID.DukeFishron)
				{
					...
					float newDamage = (0.6f * Main.GameModeInfo.EnemyDamageMultiplier);
					...
					if (flag4)
					{
						...
						if (newAI[0] >= 480f)
						{
							...
							if (Main.netMode != 1)
							{
								Projectile.NewProjectile(npc.GetSource_FromThis(null), vector.X, vector.Y, 0f, 0f, 385, 0, 0f, Main.myPlayer, 1f, (float)(npc.target + 1));
							}
							npc.netUpdate = true;
						}
					...
					else if (npc.ai[0] == 2f)
					{
						...
						if (npc.ai[2] % (float)num7 == 0f)
						{
							...
							if (Main.netMode != 1)
							{
								...
								NPC.NewNPC(npc.GetSource_FromThis(null), (int)vector6.X, (int)vector6.Y + 45, 371, 0, 0f, 0f, 0f, 0f, 255);
							}
						}
					...
					else if (npc.ai[0] == 3f)
					{
						...
						if (Main.netMode != 1 && npc.ai[2] == (float)(num9 - 30))
						{
							...
							Projectile.NewProjectile(npc.GetSource_FromThis(null), vector7.X, vector7.Y, (float)(npc.direction * 2), 8f, 385, 0, 0f, Main.myPlayer, 0f, 0f);
							Projectile.NewProjectile(npc.GetSource_FromThis(null), vector7.X, vector7.Y, (float)(-(float)npc.direction * 2), 8f, 385, 0, 0f, Main.myPlayer, 0f, 0f);
						}
						npc.ai[2] += 1f;
					...
					else if (npc.ai[0] == 7f)
					{
						...
						if (npc.ai[2] % (float)num14 == 0f)
						{
							...
							if (Main.netMode != 1)
							{
								...
								int num31 = NPC.NewNPC(npc.GetSource_FromThis(null), (int)vector10.X, (int)vector10.Y + 45, 371, 0, 0f, 0f, 0f, 0f, 255);
			...
			{
				bool configBossRushBoost = Config.BossRushXerocCurse && CalamityWorldPreTrailer.bossRushActive;
				#region Mothron
							...
							else
							{
								...
								if (npc.ai[0] == 4.2f)
								{
									...
									if (Main.netMode != 1 && vector239.Length() < 4f)
									{
										...
										{
											NPC.NewNPC(npc.GetSource_FromThis(null), num1369 * 16 + 8, num1370 * 16, 478, npc.whoAmI, 0f, 0f, 0f, 0f, 255);
										}
				...
				#endregion
				#region Pumpking
				else if (npc.type == NPCID.Pumpking && CalamityWorldPreTrailer.downedDoG)
				{
					npc.localAI[0] += 1f;
					if (npc.localAI[0] > 6f)
					{
					...
					if (Main.netMode != 1)
					{
						...
						else if (npc.ai[3] == 0f && npc.localAI[2] % 30f == 0f && npc.localAI[2] > 30f)
						{
							...
							if (!WorldGen.SolidTile((int)vector109.X / 16, (int)vector109.Y / 16))
							{
								...
								Projectile.NewProjectile(npc.GetSource_FromThis(null), vector109.X, vector109.Y, num857, num858, Main.rand.Next(326, 329), 60, 0f, Main.myPlayer, 0f, 0f);
							}
						}
					}
					if (npc.ai[0] == 0f && Main.netMode != 1)
					{
						npc.TargetClosest(true);
						npc.ai[0] = 1f;
						int num861 = NPC.NewNPC(npc.GetSource_FromThis(null), (int)(npc.position.X + (float)(npc.width / 2)), (int)npc.position.Y + npc.height / 2, 328, npc.whoAmI, 0f, 0f, 0f, 0f, 255);
						...
						num861 = NPC.NewNPC(npc.GetSource_FromThis(null), (int)(npc.position.X + (float)(npc.width / 2)), (int)npc.position.Y + npc.height / 2, 328, npc.whoAmI, 0f, 0f, 0f, 0f, 255);
						Main.npc[num861].ai[0] = 1f;
						Main.npc[num861].ai[1] = (float)npc.whoAmI;
				...
				else if (npc.type == NPCID.PumpkingBlade && CalamityWorldPreTrailer.downedDoG)
				{
					...
					if (Main.netMode != 1 && Main.npc[(int)npc.ai[1]].ai[3] == 2f)
					{
						...
						if (npc.localAI[1] > 30f)
						{
							...
							num870 *= num872;
							num871 *= num872;
							Projectile.NewProjectile(npc.GetSource_FromThis(null), npc.Center.X, npc.Center.Y, num870, num871, 329, 70, 0f, Main.myPlayer, npc.rotation, (float)npc.spriteDirection);
						}
					}
					if (Main.dayTime)
					{
				...
				#endregion
				#region IceQueen
				else if (npc.type == NPCID.IceQueen && CalamityWorldPreTrailer.downedDoG)
				{
					if (Main.dayTime)
					{
						if (npc.velocity.X > 0f)
					...
					else if (npc.ai[0] == 0f)
					{
						...
						if ((num888 < 500f || npc.ai[3] < 0f) && npc.position.Y < Main.player[npc.target].position.Y)
						{
							...
							if (npc.ai[3] == 0f && Main.netMode != 1)
							{
								...
								num893 *= num895;
								num894 *= num895;
								Projectile.NewProjectile(npc.GetSource_FromThis(null), vector116.X, vector116.Y, num893, num894, 348, 50, 0f, Main.myPlayer, 0f, 0f);
							}
						}
						else if (npc.ai[3] < 0f)
						{
						...
						if (Main.netMode != 1)
						{
							...
							if (npc.ai[3] >= (float)num901)
							{
								...
								if (!WorldGen.SolidTile(i2, j2))
								{
									...
									int num903 = Projectile.NewProjectile(npc.GetSource_FromThis(null), vector117.X, vector117.Y, speedX2, num902, 349, 44, 0f, Main.myPlayer, (float)Main.rand.Next(5), 0f);
								}
							}
						}
						if (Main.netMode != 1)
					...
					else if (npc.ai[0] == 2f)
					{
						...
						{
							npc.ai[3] = 0f;
							int num909 = Projectile.NewProjectile(npc.GetSource_FromThis(null), vector118.X, vector118.Y, num904, num905, 349, 40, 0f, Main.myPlayer, 0f, 0f);
						}
						if (Main.netMode != 1)
						{
							npc.ai[1] += (float)Main.rand.Next(1, 4);
				...
				#endregion
				#region EyeofCthulhu
				else if (npc.type == NPCID.EyeofCthulhu && (CalamityWorldPreTrailer.revenge || CalamityWorldPreTrailer.bossRushActive))
				{
					...
					else if (npc.ai[0] == 0f)
					{
						if (npc.ai[1] == 0f)
						{
							...
							else if (num16 < 500f)
							{
								...
								if (npc.ai[3] >= num18)
								{
									...
									if (Main.netMode != 1)
									{
										int num23 = NPC.NewNPC(npc.GetSource_FromThis(null), (int)vector2.X, (int)vector2.Y, 5, 0, 0f, 0f, 0f, 0f, 255);
						...
						float num29 = 0.9f;
						if (((float)npc.life < (float)npc.lifeMax * num29) || CalamityWorldPreTrailer.death)
						{
							npc.ai[0] = 1f;
							npc.ai[1] = 0f;
							npc.ai[2] = 0f;
					...
					else if (npc.ai[0] == 1f || npc.ai[0] == 2f)
					{
						...
						if (npc.ai[1] % 20f == 0f)
						{
							...
							if (Main.netMode != 1)
							{
								int num34 = NPC.NewNPC(npc.GetSource_FromThis(null), (int)vector6.X, (int)vector6.Y, 5, 0, 0f, 0f, 0f, 0f, 255);
						...
						if (npc.ai[1] == 100f)
						{
							...
							else
							{
								...
								{
									Gore.NewGore(npc.GetSource_FromThis(null),  npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), 8, 1f);
									Gore.NewGore(npc.GetSource_FromThis(null),  npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), 7, 1f);
									Gore.NewGore(npc.GetSource_FromThis(null),  npc.position, new Vector2((float)Main.rand.Next(-30, 31) * 0.2f, (float)Main.rand.Next(-30, 31) * 0.2f), 6, 1f);
									num = num35;
								}
					...
					else
					{
						...
						{
							npc.damage = (int)(20f * Main.GameModeInfo.EnemyDamageMultiplier);
						}
						...
						{
							float num46 = CalamityWorldPreTrailer.death ? 70f : 60f;
							npc.ai[2] += 1f;
							if (npc.ai[2] >= num46)
							{
								npc.velocity *= 0.96f;
							...
							int num47 = CalamityWorldPreTrailer.death ? 70 : 80;
		...
		{
			if (!CalamityWorldPreTrailer.spawnedHardBoss)
			{
				if (npc.type == NPCID.TheDestroyer || npc.type == NPCID.Spazmatism || npc.type == NPCID.Retinazer || npc.type == NPCID.SkeletronPrime ||
					npc.type == NPCID.Plantera || npc.type == Mod.Find<ModNPC>("Cryogen").Type || npc.type == Mod.Find<ModNPC>("AquaticScourgeHead").Type ||
					npc.type == Mod.Find<ModNPC>("BrimstoneElemental").Type || npc.type == Mod.Find<ModNPC>("Astrageldon").Type || npc.type == Mod.Find<ModNPC>("AstrumDeusHeadSpectral").Type ||
					npc.type == Mod.Find<ModNPC>("Calamitas").Type || npc.type == Mod.Find<ModNPC>("Siren").Type || npc.type == Mod.Find<ModNPC>("PlaguebringerGoliath").Type ||
					npc.type == Mod.Find<ModNPC>("ScavengerBody").Type || npc.type == NPCID.DukeFishron || npc.type == NPCID.CultistBoss || npc.type == NPCID.Golem)
				{
					CalamityWorldPreTrailer.spawnedHardBoss = true;
					if (Main.netMode == 2)
					{
			...
			if (CalamityWorldPreTrailer.revenge || CalamityWorldPreTrailer.bossRushActive)
			{
					...
					if (npc.ai[0] == -2f)
					{
						if ((double)Main.npc[(int)npc.ai[3]].life <= (double)Main.npc[(int)npc.ai[3]].lifeMax * 0.75)
						{
							...
							if ((int)newAI[2] % 60 == 0)
							{
								...
								if (Main.netMode != 1)
								{
									int num = NPC.NewNPC(npc.GetSource_FromThis(null), (int)npc.Center.X, (int)npc.Center.Y, 400, 0, 0f, 0f, 0f, 0f, 255);
									Main.npc[num].ai[3] = npc.ai[3];
									Main.npc[num].netUpdate = true;
								}
							}
						}
						if ((CalamityWorldPreTrailer.death || CalamityWorldPreTrailer.bossRushActive) && npc.type == NPCID.MoonLordHead)
						{
				...
				else if (npc.type == NPCID.CultistBoss)
				{
					...
					{
						if (!NPC.AnyNPCs(Mod.Find<ModNPC>("Eidolist").Type))
						{
							if (newAI[0] < 120f)
							{
								newAI[0] += 1f;
							...
							if (newAI[0] >= 120f)
							{
								...
								if (Main.netMode != 1)
								{
									NPC.NewNPC(npc.GetSource_FromThis(null), (int)npc.Center.X, (int)npc.Center.Y, Mod.Find<ModNPC>("Eidolist").Type, 0, 0f, 0f, 0f, 0f, 255);
								}
								newAI[0] = -360f;
							}
						}
							...
							if (CultCountdown == 0)
							{
								if (Main.netMode != 1)
								{
									...
									for (int i = 0; i < CultProjectiles; i++)
									{
										...
										int projectileI = Projectile.NewProjectile(npc.GetSource_FromThis(null), spawn2.X, spawn2.Y, velocity.X, velocity.Y, projectileType, damage, 0f, Main.myPlayer, 0f, 0f);
				...
				else if (npc.type == NPCID.DukeFishron)
				{
					...
					bool murderMode2 = (double)npc.life <= (double)npc.lifeMax * ((CalamityWorldPreTrailer.death || CalamityWorldPreTrailer.bossRushActive) ? 0.3 : 0.2);
					float num = 0.6f * Main.GameModeInfo.EnemyDamageMultiplier; //1.2
					bool flag3 = npc.ai[0] > 4f;
					bool flag4 = npc.ai[0] > 9f;
					if (flag4)
					{
				...
				#endregion
				#region Golem
				else if (npc.type == NPCID.GolemHeadFree && !CalamityWorldPreTrailer.bossRushActive)
				{
					bool enrage = true;
				...
				else if (npc.type == NPCID.GolemHead && !CalamityWorldPreTrailer.bossRushActive)
				{
					bool enrage = true;
					if ((double)Main.player[npc.target].Center.Y > Main.worldSurface * 16.0)
					{
				...
				else if (npc.type == NPCID.Plantera)
				{
					...
					if (npc.life < npc.lifeMax / 8 || CalamityWorldPreTrailer.bossRushActive)
					{
					...
					{
						npc.defense = 42;
						if (!jungle && !CalamityWorldPreTrailer.bossRushActive)
						{
							npc.defense = 200;
					...
					else
					{
						...
						if (newAI[0] >= ((enraged || configBossRushBoost) ? 150f : 270f))
						{
							...
							{
								NPC.NewNPC(npc.GetSource_FromThis(null), (int)npc.Center.X, (int)npc.Center.Y, NPCID.PlanterasTentacle, npc.whoAmI, 0f, 0f, 0f, 0f, 255);
							}
						}
						if (CalamityWorldPreTrailer.death || (double)npc.life < (double)npc.lifeMax * 0.25 || CalamityWorldPreTrailer.bossRushActive)
						{
							newAI[1] += 1f;
							if (CalamityWorldPreTrailer.death || CalamityWorldPreTrailer.bossRushActive)
							{
				...
				if (npc.type == NPCID.PlanterasTentacle)
				{
					bool jungle = Main.player[npc.target].ZoneJungle;
					newAI[0] += 1f;
					if (CalamityWorldPreTrailer.death || !jungle)
					{
						newAI[0] += 1f;
					}
					if (newAI[0] >= 480f)
					{
						...
						{
							NPC.NewNPC(npc.GetSource_FromThis(null), (int)npc.Center.X, (int)npc.Center.Y, NPCID.Spore, npc.whoAmI, 0f, 0f, 0f, 0f, 255);
						}
					}
				}
				#endregion
				...
				else if (npc.type == NPCID.SkeletronPrime)
				{
					...
					if (Main.netMode != 1)
					{
						if (npc.ai[1] != 1f || (!CalamityWorldPreTrailer.death && (double)npc.life > (double)npc.lifeMax * 0.25) || CalamityWorldPreTrailer.bossRushActive || targetFloatingUp)
						{
							npc.localAI[0] += 1f;
							...
							if (npc.localAI[0] >= ((enraged || configBossRushBoost) ? 90f : 120f))
							{
								...
								if (Collision.CanHit(vector16, 1, 1, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
								{
									...
									int num164 = 270;
									vector16 += value * 5f;
									int num165 = Projectile.NewProjectile(npc.GetSource_FromThis(null), vector16.X, vector16.Y, num160, num161, num164, num163, 0f, Main.myPlayer, -1f, 0f);
									Main.projectile[num165].timeLeft = 300;
								}
							}
						}
						if (CalamityWorldPreTrailer.death || (double)npc.life <= (double)npc.lifeMax * 0.25 || CalamityWorldPreTrailer.bossRushActive || targetFloatingUp)
						{
							newAI[0] += 1f;
							if (npc.ai[1] == 1f || CalamityWorldPreTrailer.bossRushActive)
							{
								newAI[0] += 3f;
							}
							if (newAI[0] >= ((enraged || configBossRushBoost) ? 180f : 240f)) //4 seconds or 1 second
							{
								...
								for (i = 0; i < 6; i++)
								{
									...
									int projectile = Projectile.NewProjectile(npc.GetSource_FromThis(null), shootFromVector.X, shootFromVector.Y, (float)(Math.Sin(offsetAngle) * 6f), (float)(Math.Cos(offsetAngle) * 6f), 100, damage, 0f, Main.myPlayer, 0f, 0f);
									int projectile2 = Projectile.NewProjectile(npc.GetSource_FromThis(null), shootFromVector.X, shootFromVector.Y, (float)(-Math.Sin(offsetAngle) * 6f), (float)(-Math.Cos(offsetAngle) * 6f), 100, damage, 0f, Main.myPlayer, 0f, 0f);
					...
					if (npc.ai[1] == 1f)
					{
						int speed = CalamityWorldPreTrailer.bossRushActive ? 7 : 3;
					...
					if (Main.netMode != 1)
					{
						...
						if (npc.localAI[1] > ((enraged || configBossRushBoost) ? 90f : 120f))
						{
							...
							if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
							{
								...
								int num947 = Projectile.NewProjectile(npc.GetSource_FromThis(null), vector104.X, vector104.Y, num942, num943, num946, num945, 0f, Main.myPlayer, 0f, 0f);
				...
				else if (npc.type == NPCID.Retinazer)
				{
					...
					if (npc.ai[0] == 0f)
					{
						double healthMult = ((CalamityWorldPreTrailer.death || CalamityWorldPreTrailer.bossRushActive) ? 0.8 : 0.6);
					...
					else if (npc.ai[0] != 1f && npc.ai[0] != 2f && npc.ai[0] != 0f)
					{
						...
						if (newAI[0] >= ((enraged || configBossRushBoost) ? 180f : 240f))
						{
							...
							if (Main.netMode != 1)
							{
								...
								vector34.X += num349;
								vector34.Y += num350;
								Projectile.NewProjectile(npc.GetSource_FromThis(null), vector34.X, vector34.Y, num349, num350, num355, num354, 0f, Main.myPlayer, 0f, 0f);
							}
						}
					}
				}
					...
					if (Main.netMode != 1)
					{
						...
						if (newAI[0] >= (float)Main.rand.Next(1400, 26000))
						{
							...
							if (Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
							{
								...
								if ((double)npc.life <= (double)npc.lifeMax * 0.4 || CalamityWorldPreTrailer.bossRushActive)
								{
									speed += CalamityWorldPreTrailer.death ? 0.33f : 0.25f;
								}
								if ((double)npc.life <= (double)npc.lifeMax * 0.1 || CalamityWorldPreTrailer.bossRushActive)
								{
									speed += CalamityWorldPreTrailer.death ? 0.33f : 0.25f;
								...
								int num9 = 28;
								int num10 = 100;
								int value = (CalamityWorldPreTrailer.bossRushActive ? 3 : 4);
								if (Main.rand.Next(value) == 0 || enraged || configBossRushBoost)
								{
									int secondValue = (newAI[2] > 0f ? 3 : 5);
									num10 = (Main.rand.Next(secondValue) == 0 ? Mod.Find<ModProjectile>("DestroyerHomingLaser").Type : 257);
									num9 = 31;
								}
								...
								vector.X += num6 * 5f;
								vector.Y += num7 * 5f;
								int num11 = Projectile.NewProjectile(npc.GetSource_FromThis(null), vector.X, vector.Y, num6, num7, num10, num9, 0f, Main.myPlayer, 0f, 0f);
								if (num10 != Mod.Find<ModProjectile>("DestroyerHomingLaser").Type)
								{
				...
				else if (npc.type == NPCID.WallofFleshEye)
				{
					if (NPC.CountNPCS(NPCID.WallofFlesh) > 0)
					{
						...
						if (Main.netMode != 1)
						{
							...
							if ((double)Main.npc[Main.wofNPCIndex].life < (double)Main.npc[Main.wofNPCIndex].lifeMax * 0.1 || CalamityWorldPreTrailer.bossRushActive)
							{
								npc.localAI[3] += 2f;
								num352 += 3;
							}
				...
				else if (npc.type == NPCID.SkeletronHead || npc.type == NPCID.DungeonGuardian)
				{
					if (npc.ai[1] == 1f)
					{
						if (Main.netMode != 1)
						{
							...
							if ((double)npc.life <= (double)npc.lifeMax * 0.15 || CalamityWorldPreTrailer.bossRushActive)
							{
								npc.localAI[1] += 3f;
							}
							if (CalamityWorldPreTrailer.death || CalamityWorldPreTrailer.bossRushActive)
						...
						float num176 = CalamityWorldPreTrailer.bossRushActive ? 10f : 5f;
					...
					else if (npc.ai[0] == 3f)
					{
						...
						if (flag39 && npc.position.Y + (float)npc.height < Main.player[npc.target].position.Y && Collision.CanHit(vector74, 1, 1, Main.player[npc.target].position, Main.player[npc.target].width, Main.player[npc.target].height))
						{
							...
							if (Main.netMode != 1)
							{
								...
								int num606 = 13;
								int num607 = 55;
								int num608 = Projectile.NewProjectile(npc.GetSource_FromThis(null), vector74.X, vector74.Y, num603, num604, num607, num606, 0f, Main.myPlayer, 0f, 0f);
				...
				else if (npc.type == NPCID.BrainofCthulhu)
				{
					...
					if (npc.ai[0] < 0f)
					{
						npc.knockBackResist = (CalamityWorldPreTrailer.death ? 0.05f : 0.1f) * Main.GameModeInfo.KnockbackToEnemiesMultiplier;
				...
				else if (npc.type == NPCID.EaterofWorldsHead && !CalamityWorldPreTrailer.bossRushActive)
				{
					if (!Main.player[npc.target].dead)
					{
						newAI[0] += 1f;
		...
		{
			if (pearlAura && !CalamityPlayerPreTrailer.areThereAnyDamnBosses)
			{
				npc.velocity.X *= 0.95f;
				npc.velocity.Y *= 0.95f;
			}
			if (silvaStun && !CalamityWorldPreTrailer.bossRushActive)
			{
				npc.velocity.X = 0f;
				npc.velocity.Y = 0f;
			}
		...
		{
			if (target.GetModPlayer<CalamityPlayerPreTrailer>().snowman)
			{
				if (npc.type == NPCID.Demon || npc.type == NPCID.VoodooDemon || npc.type == NPCID.RedDevil)
				{
					target.AddBuff(Mod.Find<ModBuff>("PopoNoseless").Type, 36000);
				}
			}
			if (npc.type == NPCID.GolemHead)
			{
				target.AddBuff(Mod.Find<ModBuff>("ArmorCrunch").Type, 300);
			}
			...
			if (CalamityWorldPreTrailer.revenge)
			{
				...
				{
					target.AddBuff(Mod.Find<ModBuff>("MarkedforDeath").Type, 60);
				}
				...
				{
					target.AddBuff(Mod.Find<ModBuff>("MarkedforDeath").Type, 240);
				}
				...
				{
					target.AddBuff(Mod.Find<ModBuff>("MarkedforDeath").Type, 360);
				}
				else if (npc.type == NPCID.DukeFishron)
				{
					target.AddBuff(Mod.Find<ModBuff>("MarkedforDeath").Type, 180);
		...
		{
			if (npc.type == NPCID.TheDestroyerBody || npc.type == Mod.Find<ModNPC>("AquaticScourgeBody").Type || npc.type == Mod.Find<ModNPC>("AquaticScourgeBodyAlt").Type)
			{
				if (projectile.type == ProjectileID.RainFriendly || projectile.type == ProjectileID.BloodRain ||
					projectile.type == Mod.Find<ModProjectile>("Shaderain").Type || projectile.aiStyle == 4 ||
					projectile.type == ProjectileID.CrystalPulse2 || projectile.type == ProjectileID.HallowStar ||
					projectile.type == Mod.Find<ModProjectile>("FossilShardThrown").Type)
					projectile.damage = (int)((double)projectile.damage * 0.65);
			}
			if (Main.player[projectile.owner].GetModPlayer<CalamityPlayerPreTrailer>().eGauntlet)
			{
				if (projectile.CountsAsClass(DamageClass.Melee) && npc.damage > 0 && !npc.boss && !npc.friendly && !npc.dontTakeDamage && Main.rand.Next(15) == 0 && npc.type != 477 &&
					npc.type != 327 && npc.type != 135 && npc.type != 136 && npc.type != 325 && npc.type != 344 && npc.type != 346 && npc.type != 345 &&
					npc.type != Mod.Find<ModNPC>("Reaper").Type && npc.type != Mod.Find<ModNPC>("Mauler").Type && npc.type != Mod.Find<ModNPC>("EidolonWyrmHead").Type &&
					npc.type != Mod.Find<ModNPC>("EidolonWyrmHeadHuge").Type && npc.type != Mod.Find<ModNPC>("ColossalSquid").Type && npc.type != NPCID.DD2Betsy)
				{
					if (!CalamityPlayerPreTrailer.areThereAnyDamnBosses)
						projectile.damage = npc.lifeMax * 3;
				}
			}
			if (Main.player[projectile.owner].GetModPlayer<CalamityPlayerPreTrailer>().eTalisman)
			{
				if (projectile.CountsAsClass(DamageClass.Magic) && npc.damage > 0 && !npc.boss && !npc.friendly && !npc.dontTakeDamage && Main.rand.Next(15) == 0 && npc.type != 477 &&
					npc.type != 327 && npc.type != 135 && npc.type != 136 && npc.type != 325 && npc.type != 344 && npc.type != 346 && npc.type != 345 &&
					npc.type != Mod.Find<ModNPC>("Reaper").Type && npc.type != Mod.Find<ModNPC>("Mauler").Type && npc.type != Mod.Find<ModNPC>("EidolonWyrmHead").Type &&
					npc.type != Mod.Find<ModNPC>("EidolonWyrmHeadHuge").Type && npc.type != Mod.Find<ModNPC>("ColossalSquid").Type && npc.type != NPCID.DD2Betsy)
				{
			...
			if (Main.player[projectile.owner].GetModPlayer<CalamityPlayerPreTrailer>().nanotech)
			{
				if (projectile.GetGlobalProjectile<CalamityGlobalProjectile>().rogue &&
			...
			if (Main.player[projectile.owner].GetModPlayer<CalamityPlayerPreTrailer>().eQuiver)
			{
				if (projectile.CountsAsClass(DamageClass.Ranged) && npc.damage > 0 && !npc.boss && !npc.friendly && !npc.dontTakeDamage && Main.rand.Next(15) == 0 && npc.type != 477 &&
					npc.type != 327 && npc.type != 135 && npc.type != 136 && npc.type != 325 && npc.type != 344 && npc.type != 346 && npc.type != 345 &&
					npc.type != Mod.Find<ModNPC>("Reaper").Type && npc.type != Mod.Find<ModNPC>("Mauler").Type && npc.type != Mod.Find<ModNPC>("EidolonWyrmHead").Type &&
					npc.type != Mod.Find<ModNPC>("EidolonWyrmHeadHuge").Type && npc.type != Mod.Find<ModNPC>("ColossalSquid").Type && npc.type != NPCID.DD2Betsy)
				{
			...
			if (Main.player[projectile.owner].GetModPlayer<CalamityPlayerPreTrailer>().statisBeltOfCurses)
			{
				if ((projectile.minion || CalamityModClassicPreTrailer.projectileMinionList.Contains(projectile.type)) && npc.damage > 0 && !npc.boss && !npc.friendly && !npc.dontTakeDamage &&
					Main.rand.Next(15) == 0 && npc.type != 477 && npc.type != 327 && npc.type != 135 && npc.type != 136 && npc.type != 325 &&
					npc.type != 344 && npc.type != 346 && npc.type != 345 && npc.type != Mod.Find<ModNPC>("Reaper").Type && npc.type != Mod.Find<ModNPC>("Mauler").Type &&
					npc.type != Mod.Find<ModNPC>("EidolonWyrmHead").Type && npc.type != Mod.Find<ModNPC>("EidolonWyrmHeadHuge").Type && npc.type != Mod.Find<ModNPC>("ColossalSquid").Type && npc.type != NPCID.DD2Betsy)
				{
					if (!CalamityPlayerPreTrailer.areThereAnyDamnBosses)
						projectile.damage = npc.lifeMax * 3;
				}
		...
		{
			if (player.GetModPlayer<CalamityPlayerPreTrailer>().bloodflareSet)
			{
				if (!npc.SpawnedFromStatue && npc.damage > 0 && ((double)npc.life < (double)npc.lifeMax * 0.5) &&
					player.GetModPlayer<CalamityPlayerPreTrailer>().bloodflareHeartTimer <= 0)
				{
					player.GetModPlayer<CalamityPlayerPreTrailer>().bloodflareHeartTimer = 180;
					...
					player.GetModPlayer<CalamityPlayerPreTrailer>().bloodflareManaTimer <= 0)
				{
					player.GetModPlayer<CalamityPlayerPreTrailer>().bloodflareManaTimer = 180;
					Item.NewItem(npc.GetSource_FromThis(null), (int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, 184, 1, false, 0, false, false);
				}
			}
		}
		#endregion
		...
		{
			if (Main.player[projectile.owner].GetModPlayer<CalamityPlayerPreTrailer>().sGenerator)
			{
				if (projectile.minion && npc.damage > 0)
				{
					switch (Main.rand.Next(3))
					{
						case 0: Main.player[projectile.owner].AddBuff(Mod.Find<ModBuff>("SpiritGeneratorAtkBuff").Type, 120); break;
						case 1: Main.player[projectile.owner].AddBuff(Mod.Find<ModBuff>("SpiritGeneratorRegenBuff").Type, 120); break;
						case 2: Main.player[projectile.owner].AddBuff(Mod.Find<ModBuff>("SpiritGeneratorDefBuff").Type, 120); break;
					}
				}
			}
			if (Main.player[projectile.owner].GetModPlayer<CalamityPlayerPreTrailer>().bloodflareSet)
			{
				if (!npc.SpawnedFromStatue && npc.damage > 0 && ((double)npc.life < (double)npc.lifeMax * 0.5) &&
					Main.player[projectile.owner].GetModPlayer<CalamityPlayerPreTrailer>().bloodflareHeartTimer <= 0)
				{
					Main.player[projectile.owner].GetModPlayer<CalamityPlayerPreTrailer>().bloodflareHeartTimer = 180;
					Item.NewItem(npc.GetSource_FromThis(null), (int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, 58, 1, false, 0, false, false);
				}
				else if (!npc.SpawnedFromStatue && npc.damage > 0 && ((double)npc.life > (double)npc.lifeMax * 0.5) &&
					Main.player[projectile.owner].GetModPlayer<CalamityPlayerPreTrailer>().bloodflareManaTimer <= 0)
				{
					Main.player[projectile.owner].GetModPlayer<CalamityPlayerPreTrailer>().bloodflareManaTimer = 180;
					Item.NewItem(npc.GetSource_FromThis(null), (int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, 184, 1, false, 0, false, false);
				}
			}
		}
		...
		public override bool CheckDead(NPC npc)
		{
			if (npc.lifeMax > 1000 && npc.type != 288 &&
				npc.type != Mod.Find<ModNPC>("PhantomSpirit").Type &&
				...
				Main.player[npc.target].ZoneDungeon)
			{
				...
				if (Main.rand.Next(maxValue) == 0 && Main.wallDungeon[(int)Main.tile[(int)npc.Center.X / 16, (int)npc.Center.Y / 16].WallType])
				{
					int randomType = Main.rand.Next(4);
					if (randomType == 0)
					{
						randomType = Mod.Find<ModNPC>("PhantomSpirit").Type;
					}
					else if (randomType == 1)
					{
						randomType = Mod.Find<ModNPC>("PhantomSpiritS").Type;
					}
					else if (randomType == 2)
					{
						randomType = Mod.Find<ModNPC>("PhantomSpiritM").Type;
					}
					else
					{
						randomType = Mod.Find<ModNPC>("PhantomSpiritL").Type;
					}
					NPC.NewNPC(npc.GetSource_FromThis(null), (int)npc.Center.X, (int)npc.Center.Y, randomType, 0, 0f, 0f, 0f, 0f, 255);
				}
			}
			return true;
		}
					...
					case NPCID.MotherSlime:
						if (npc.life <= 0)
						{
							if (Main.netMode != 1)
							{
								...
								{
									int num263 = NPC.NewNPC(npc.GetSource_FromThis(null), (int)(npc.position.X + (float)(npc.width / 2)), (int)(npc.position.Y + (float)npc.height), 1, 0, 0f, 0f, 0f, 0f, 255);
									Main.npc[num263].SetDefaults(-5, default);
		...
		{
			if (player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneSulphur)
			{
			...
			else if (player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneAbyss)
			{
				spawnRate = (int)((double)spawnRate * 0.7);
				maxSpawns = (int)((float)maxSpawns * 1.1f);
			}
			else if (player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneCalamity)
			{
				spawnRate = (int)((double)spawnRate * 0.9);
				maxSpawns = (int)((float)maxSpawns * 1.1f);
			}
			else if (player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneAstral)
			{
				spawnRate = (int)((double)spawnRate * 0.6);
				maxSpawns = (int)((float)maxSpawns * 1.2f);
			}
			else if (player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneSunkenSea)
			{
				spawnRate = (int)((double)spawnRate * 0.9);
				maxSpawns = (int)((float)maxSpawns * 1.1f);
			}
			...
			if (CalamityWorldPreTrailer.demonMode)
			{
				spawnRate = (int)((double)spawnRate * 0.75);
			}
			if (player.GetModPlayer<CalamityPlayerPreTrailer>().clamity)
			{
				spawnRate = (int)((double)spawnRate * 0.02);
				maxSpawns = (int)((float)maxSpawns * 1.5f);
			}
			if (player.GetModPlayer<CalamityPlayerPreTrailer>().zerg && player.GetModPlayer<CalamityPlayerPreTrailer>().chaosCandle)
			{
				spawnRate = (int)((double)spawnRate * 0.005);
				maxSpawns = (int)((float)maxSpawns * 7.5f);
			}
			else if (player.GetModPlayer<CalamityPlayerPreTrailer>().zerg)
			{
				spawnRate = (int)((double)spawnRate * 0.01);
				maxSpawns = (int)((float)maxSpawns * 5f);
			}
			else if (player.GetModPlayer<CalamityPlayerPreTrailer>().chaosCandle)
			{
				spawnRate = (int)((double)spawnRate * 0.02);
				maxSpawns = (int)((float)maxSpawns * 2.5f);
			}
			if (player.GetModPlayer<CalamityPlayerPreTrailer>().zen && player.GetModPlayer<CalamityPlayerPreTrailer>().tranquilityCandle)
			{
				spawnRate = (int)((double)spawnRate * (player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneAbyss ? 1.75 : 75));
				maxSpawns = (int)((float)maxSpawns * (player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneAbyss ? 0.625f : 0.005f));
			}
			else if (CalamityPlayerPreTrailer.areThereAnyDamnBosses || player.GetModPlayer<CalamityPlayerPreTrailer>().zen || (Config.DisableExpertEnemySpawnsNearHouse && player.townNPCs > 1f && Main.expertMode))
			{
				spawnRate = (int)((double)spawnRate * (player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneAbyss ? 1.5 : 50));
				maxSpawns = (int)((float)maxSpawns * (player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneAbyss ? 0.75f : 0.01f));
			}
			else if (player.GetModPlayer<CalamityPlayerPreTrailer>().tranquilityCandle)
			{
				spawnRate = (int)((double)spawnRate * (player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneAbyss ? 1.25 : 25));
				maxSpawns = (int)((float)maxSpawns * (player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneAbyss ? 0.875f : 0.02f));
			}
		}
		#endregion
		...
		{
			if (player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneAbyss)
			{
		...
		{
			if (spawnInfo.Player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneAbyss ||
				spawnInfo.Player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneCalamity ||
				...
				(spawnInfo.Player.GetModPlayer<CalamityPlayerPreTrailer>().ZoneAstral && !NPC.LunarApocalypseIsUp))
			{
				pool[0] = 0f;
			}
		}
		...
		public override void DrawEffects(NPC npc, ref Color drawColor)
		{
			...
			if (bFlames || enraged)
			{
				if (Main.rand.Next(5) < 4)
				{
					int dust = Dust.NewDust(npc.position - new Vector2(2f, 2f), npc.width + 4, npc.height + 4, ModContent.DustType<BrimstoneFlame>(), npc.velocity.X * 0.4f, npc.velocity.Y * 0.4f, 100, default(Color), 3.5f);
					Main.dust[dust].noGravity = true;
					Main.dust[dust].velocity *= 1.8f;
					Main.dust[dust].velocity.Y -= 0.5f;
					if (Main.rand.Next(4) == 0)
			...
			if (hFlames)
			{
				if (Main.rand.Next(5) < 4)
				{
					int dust = Dust.NewDust(npc.position - new Vector2(2f, 2f), npc.width + 4, npc.height + 4, ModContent.DustType<HolyFlame>(), npc.velocity.X * 0.4f, npc.velocity.Y * 0.4f, 100, default(Color), 3.5f);
					Main.dust[dust].noGravity = true;
					Main.dust[dust].velocity *= 1.8f;
					Main.dust[dust].velocity.Y -= 0.5f;
					if (Main.rand.Next(4) == 0)
		...
		public override Color? GetAlpha(NPC npc, Color drawColor)
		{
			...
			if (enraged || (Config.BossRushXerocCurse && CalamityWorldPreTrailer.bossRushActive))
		...
		public override void GetChat(NPC npc, ref string chat)
		{
			switch (npc.type)
			{
				case NPCID.Guide:
					...
					if (Main.rand.Next(20) == 0 && CalamityWorldPreTrailer.spawnAstralMeteor)
					...
					if (Main.rand.Next(20) == 0 && CalamityWorldPreTrailer.downedProvidence)
				...
				case NPCID.PartyGirl:
					int fapsol = NPC.FindFirstNPC(Mod.Find<ModNPC>("FAP").Type);
				...
				case NPCID.Wizard:
					...
					if (Main.rand.Next(10) == 0 && CalamityWorldPreTrailer.spawnAstralMeteor)
						chat = "Space just got way too close for comfort.";
					break;
				case NPCID.Dryad:
					if (Main.rand.Next(5) == 0 && CalamityWorldPreTrailer.buffedEclipse && Main.eclipse)
					...
					break;
				case NPCID.Stylist:
					int fapsol2 = NPC.FindFirstNPC(Mod.Find<ModNPC>("FAP").Type);
					...
					if (Main.rand.Next(15) == 0 && CalamityWorldPreTrailer.spawnAstralMeteor)
				...
				case NPCID.ArmsDealer:
					...
					if (Main.rand.Next(10) == 0 && CalamityWorldPreTrailer.downedDoG)
				...
				case NPCID.Merchant:
					...
					if (Main.rand.Next(5) == 0 && Main.raining && CalamityWorldPreTrailer.sulphurTiles > 30)
						chat = "If this acid rain keeps up, there'll be a shortage of Dirt Blocks soon enough!";
					break;
				case NPCID.Mechanic:
					int fapsol3 = NPC.FindFirstNPC(Mod.Find<ModNPC>("FAP").Type);
					if (Main.rand.Next(5) == 0 && NPC.downedMoonlord)
				...
				case NPCID.DD2Bartender:
					int fapsol4 = NPC.FindFirstNPC(Mod.Find<ModNPC>("FAP").Type);
					...
					break;
				case NPCID.Pirate:
					if (Main.rand.Next(5) == 0 && !CalamityWorldPreTrailer.downedLeviathan)
					...
					if (Main.rand.Next(5) == 0 && CalamityWorldPreTrailer.downedAquaticScourge)
				...
				case NPCID.Cyborg:
					...
					if (Main.rand.Next(10) == 0 && !CalamityWorldPreTrailer.downedPlaguebringer && NPC.downedGolemBoss)
				...
				case NPCID.Clothier:
					...
					if (Main.rand.Next(10) == 0 && CalamityWorldPreTrailer.downedPolterghast)
				...
				case NPCID.DyeTrader:
					int permadong2 = NPC.FindFirstNPC(Mod.Find<ModNPC>("DILF").Type);
					if (Main.rand.Next(5) == 0)
				...
				case NPCID.TaxCollector:
					...
					if (Main.rand.Next(5) == 0 && !CalamityWorldPreTrailer.downedBrimstoneElemental)
				...
				case NPCID.Demolitionist:
					if (Main.rand.Next(5) == 0 && CalamityWorldPreTrailer.downedDoG)
		...
		{
			if (shop.NpcType == NPCID.Merchant && (Main.player[Main.myPlayer].HasItem(Mod.Find<ModItem>("FirestormCannon").Type) || Main.player[Main.myPlayer].HasItem(Mod.Find<ModItem>("SpectralstormCannon").Type)))
			{
			...
			if (shop.NpcType == NPCID.Merchant)
			{
				...
				if (NPC.downedBoss2)
				{
					...
					if (WorldGen.crimson)
					{
						shop.Add(ItemID.Vilethorn);
					}
					else
					{
						shop.Add(ItemID.TheRottedFork);
					}
				}
				if (NPC.downedBoss3)
				{
			...
			if (shop.NpcType == NPCID.ArmsDealer && (Main.player[Main.myPlayer].HasItem(Mod.Find<ModItem>("Impaler").Type)))
			{
				shop.Add(ItemID.Stake);
			}
			if (shop.NpcType == NPCID.ArmsDealer) 
			{
				if (NPC.downedBoss2)
				{
					if (WorldGen.crimson)
					{
						shop.Add(ItemID.Musket);
					}
					else
					{
						shop.Add(ItemID.TheUndertaker);
					}
				}
			}
			if (shop.NpcType == NPCID.Dryad)
			{
				shop.AddWithCustomValue(ItemID.JungleRose, Item.buyPrice(0, 2, 0, 0));
				shop.AddWithCustomValue(ItemID.NaturesGift, Item.buyPrice(0, 10, 0, 0));
				shop.AddWithCustomValue(ItemID.GoblinBattleStandard, Item.buyPrice(0, 1, 0, 0));
				if (NPC.downedSlimeKing)
				{
					shop.AddWithCustomValue(ItemID.SlimeCrown, Item.buyPrice(0, 2, 0, 0));
				}
				if (CalamityWorldPreTrailer.downedDesertScourge)
				{
					shop.AddWithCustomValue(Mod.Find<ModItem>("DriedSeafood").Type, Item.buyPrice(0, 2, 0, 0));
				}
				if (NPC.downedBoss1)
				{
					shop.AddWithCustomValue(ItemID.SuspiciousLookingEye, Item.buyPrice(0, 3, 0, 0));
				}
				if (CalamityWorldPreTrailer.downedCrabulon)
				{
					shop.AddWithCustomValue(Mod.Find<ModItem>("DecapoditaSprout").Type, Item.buyPrice(0, 4, 0, 0));
				}
				if (NPC.downedBoss2)
				{
					shop.AddWithCustomValue(ItemID.BloodySpine, Item.buyPrice(0, 6, 0, 0));
					shop.AddWithCustomValue(ItemID.WormFood, Item.buyPrice(0, 6, 0, 0));
					if (WorldGen.crimson)
					{
						if (Main.expertMode)
						{
							shop.Add(ItemID.WormScarf);
						}
						shop.Add(ItemID.BandofStarpower);
					}
					else
					{
						if (Main.expertMode)
						{
							shop.Add(ItemID.BrainOfConfusion);
						}
						shop.Add(ItemID.PanicNecklace);
					}
				}
				if (CalamityWorldPreTrailer.downedPerforator)
				{
					shop.AddWithCustomValue(Mod.Find<ModItem>("BloodyWormFood").Type, Item.buyPrice(0, 10, 0, 0));
					if (Main.expertMode)
					{
						shop.Add(Mod.Find<ModItem>("RottenBrain").Type);
					}
				}
				if (CalamityWorldPreTrailer.downedHiveMind)
				{
					shop.AddWithCustomValue(Mod.Find<ModItem>("Teratoma").Type, Item.buyPrice(0, 10, 0, 0));
					if (Main.expertMode)
					{
						shop.Add(Mod.Find<ModItem>("BloodyWormTooth").Type);
					}
				}
				if (CalamityWorldPreTrailer.downedSlimeGod)
				{
					shop.AddWithCustomValue(Mod.Find<ModItem>("OverloadedSludge").Type, Item.buyPrice(0, 15, 0, 0));
				}
				if (CalamityWorldPreTrailer.downedAquaticScourge)
				{
					shop.AddWithCustomValue(Mod.Find<ModItem>("Seafood").Type, Item.buyPrice(0, 20, 0, 0));
				}
				if (NPC.downedHalloweenKing)
				{
					shop.AddWithCustomValue(ItemID.PumpkinMoonMedallion, Item.buyPrice(0, 25, 0, 0));
				}
				if (NPC.downedChristmasIceQueen)
				{
					shop.AddWithCustomValue(ItemID.NaughtyPresent, Item.buyPrice(0, 25, 0, 0));
				}
				shop.Add(Mod.Find<ModItem>("RomajedaOrchid").Type);
			}
			if (shop.NpcType == NPCID.GoblinTinkerer)
			{
				shop.AddWithCustomValue(Mod.Find<ModItem>("MeleeLevelMeter").Type, Item.buyPrice(0, 5, 0, 0));
				shop.AddWithCustomValue(Mod.Find<ModItem>("RangedLevelMeter").Type, Item.buyPrice(0, 5, 0, 0));
				shop.AddWithCustomValue(Mod.Find<ModItem>("MagicLevelMeter").Type, Item.buyPrice(0, 5, 0, 0));
				shop.AddWithCustomValue(Mod.Find<ModItem>("SummonLevelMeter").Type, Item.buyPrice(0, 5, 0, 0));
				shop.AddWithCustomValue(Mod.Find<ModItem>("RogueLevelMeter").Type, Item.buyPrice(0, 5, 0, 0));
			}
			if (shop.NpcType == NPCID.Clothier)
			{
				shop.AddWithCustomValue(Mod.Find<ModItem>("BlueBrickWallUnsafe").Type, Item.buyPrice(0, 0, 0, 10));
				shop.AddWithCustomValue(Mod.Find<ModItem>("BlueSlabWallUnsafe").Type, Item.buyPrice(0, 0, 0, 10));
				shop.AddWithCustomValue(Mod.Find<ModItem>("BlueTiledWallUnsafe").Type, Item.buyPrice(0, 0, 0, 10));
				shop.AddWithCustomValue(Mod.Find<ModItem>("GreenBrickWallUnsafe").Type, Item.buyPrice(0, 0, 0, 10));
				shop.AddWithCustomValue(Mod.Find<ModItem>("GreenSlabWallUnsafe").Type, Item.buyPrice(0, 0, 0, 10));
				shop.AddWithCustomValue(Mod.Find<ModItem>("GreenTiledWallUnsafe").Type, Item.buyPrice(0, 0, 0, 10));
				shop.AddWithCustomValue(Mod.Find<ModItem>("PinkBrickWallUnsafe").Type, Item.buyPrice(0, 0, 0, 10));
				shop.AddWithCustomValue(Mod.Find<ModItem>("PinkSlabWallUnsafe").Type, Item.buyPrice(0, 0, 0, 10));
				shop.AddWithCustomValue(Mod.Find<ModItem>("PinkTiledWallUnsafe").Type, Item.buyPrice(0, 0, 0, 10));
			}
			if (shop.NpcType == NPCID.Steampunker)
			{
				if (NPC.downedMechBoss1)
				{
					shop.AddWithCustomValue(ItemID.MechanicalWorm, Item.buyPrice(0, 20, 0, 0));
				}
				if (NPC.downedMechBoss2)
				{
					shop.AddWithCustomValue(ItemID.MechanicalEye, Item.buyPrice(0, 20, 0, 0));
				}
				if (NPC.downedMechBoss3)
				{
					shop.AddWithCustomValue(ItemID.MechanicalSkull, Item.buyPrice(0, 20, 0, 0));
				}
				Condition inAstral = CalamityHelper.Create("Player is in the Astral Infection", () => Main.player[Main.myPlayer].GetModPlayer<CalamityPlayerPreTrailer>().ZoneAstral && CalamityWorldPreTrailer.spawnAstralMeteor);
				shop.AddWithCustomValue(Mod.Find<ModItem>("AstralSolution").Type, Item.buyPrice(0, 0, 5, 0), inAstral);
			}
			if (shop.NpcType == NPCID.Wizard)
			{
				if (CalamityWorldPreTrailer.downedCryogen)
				{
					shop.AddWithCustomValue(Mod.Find<ModItem>("CryoKey").Type, Item.buyPrice(0, 15, 0, 0));
				}
				if (CalamityWorldPreTrailer.downedBrimstoneElemental)
				{
					shop.AddWithCustomValue(Mod.Find<ModItem>("CharredIdol").Type, Item.buyPrice(0, 20, 0, 0));
				}
				if (CalamityWorldPreTrailer.downedAstrageldon)
				{
					shop.AddWithCustomValue(Mod.Find<ModItem>("AstralChunk").Type, Item.buyPrice(0, 25, 0, 0));
				}
				if (CalamityWorldPreTrailer.downedStarGod)
				{
					shop.AddWithCustomValue(ItemID.SpectreStaff, Item.buyPrice(0, 25, 0, 0));
					shop.AddWithCustomValue(ItemID.InfernoFork, Item.buyPrice(0, 25, 0, 0));
					shop.AddWithCustomValue(ItemID.ShadowbeamStaff, Item.buyPrice(0, 25, 0, 0));
				}
				if (NPC.downedMoonlord)
				{
					shop.AddWithCustomValue(ItemID.CelestialSigil, Item.buyPrice(3, 0, 0, 0));
				}
				if (CalamityWorldPreTrailer.downedGuardians)
				{
					shop.AddWithCustomValue(Mod.Find<ModItem>("ProfanedShard").Type, Item.buyPrice(10, 0, 0, 0));
				}
			}
			if (shop.NpcType == NPCID.WitchDoctor)
			{
				shop.AddWithCustomValue(ItemID.Abeemination, Item.buyPrice(0, 8, 0, 0));
				if (NPC.downedPlantBoss)
				{
					shop.AddWithCustomValue(Mod.Find<ModItem>("BulbofDoom").Type, Item.buyPrice(0, 20, 0, 0));
				}
				if (NPC.downedGolemBoss)
				{
					shop.AddWithCustomValue(ItemID.SolarTablet, Item.buyPrice(0, 25, 0, 0));
					shop.AddWithCustomValue(ItemID.LihzahrdPowerCell, Item.buyPrice(0, 30, 0, 0));
				}
				if (CalamityWorldPreTrailer.downedScavenger)
				{
					shop.AddWithCustomValue(Mod.Find<ModItem>("AncientMedallion").Type, Item.buyPrice(0, 50, 0, 0));
				}
				if (CalamityWorldPreTrailer.downedPlaguebringer)
				{
					shop.AddWithCustomValue(Mod.Find<ModItem>("Abomination").Type, Item.buyPrice(0, 50, 0, 0));
				}
				if (CalamityWorldPreTrailer.downedBumble)
				{
					shop.AddWithCustomValue(Mod.Find<ModItem>("BirbPheromones").Type, Item.buyPrice(5, 0, 0, 0));
				}
			}
			if (shop.NpcType == NPCID.TravellingMerchant && Main.moonPhase == 0)
			{
				shop.Add(Mod.Find<ModItem>("FrostBarrier").Type);
			}
		}
		#endregion
		...
		public static bool AnyBossNPCS()
		{
			...
			for (int i = 0; i < 200; i++)
			{
				if (Main.npc[i].active && Main.npc[i].type != NPCID.MartianSaucerCore &&
					(Main.npc[i].boss || Main.npc[i].type == NPCID.EaterofWorldsHead || Main.npc[i].type == NPCID.EaterofWorldsTail || Main.npc[i].type == mod.Find<ModNPC>("SlimeGodRun").Type ||
					Main.npc[i].type == mod.Find<ModNPC>("SlimeGodRunSplit").Type || Main.npc[i].type == mod.Find<ModNPC>("SlimeGod").Type || Main.npc[i].type == mod.Find<ModNPC>("SlimeGodSplit").Type))
				{
					return true;
				}
			}
		...
		public static void OldDukeSpawn(int plr, int Type)
		{
			...
			while (m < 1000)
			{
				...
				{
					int num8 = NPC.NewNPC(player.GetSource_FromThis(null), (int)projectile.Center.X, (int)projectile.Center.Y + 100, mod.Find<ModNPC>("OldDuke").Type, 0, 0f, 0f, 0f, 0f, 255);
