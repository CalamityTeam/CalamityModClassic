...
namespace CalamityModClassicPreTrailer.NPCs.DesertScourge
{
	public class DesertScourgeBody : ModNPC
	{
		...
		public override void AI()
		{
            ...
            if (Main.netMode != 1 && (CalamityWorldPreTrailer.revenge || CalamityWorldPreTrailer.bossRushActive))
            {
                ...
                if (NPC.localAI[0] >= (float)Main.rand.Next(1400, 26000))
                {
                    ...
                    if (Collision.CanHit(NPC.position, NPC.width, NPC.height, Main.player[NPC.target].position, Main.player[NPC.target].width, Main.player[NPC.target].height))
                    {
                        ...
                        int projectileType = Mod.Find<ModProjectile>("SandBlast").Type;
                        int damage = 15;
                        float num941 = (NPC.GetGlobalNPC<CalamityGlobalNPC>().enraged ? 15f : 6f);
		...
		public override void HitEffect(NPC.HitInfo hit)
		{
			...
			if (NPC.life <= (NPC.lifeMax * 0.75f) && NPC.CountNPCS(Mod.Find<ModNPC>("DriedSeekerHead").Type) < wormCount)
			{
				if (Main.rand.Next(10) == 0 && Main.netMode != 1)
				{
					...
					int seeker = NPC.NewNPC(NPC.GetSource_FromThis(null), (int)spawnAt.X, (int)spawnAt.Y, Mod.Find<ModNPC>("DriedSeekerHead").Type);
			...
			if (NPC.life <= 0)
			{
				if (Main.netMode != NetmodeID.Server)
				{
					...
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position,
						NPC.velocity * randomSpread * Main.rand.NextFloat(), Mod.Find<ModGore>("ScourgeBody").Type, 1f);
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position,
						NPC.velocity * randomSpread * Main.rand.NextFloat(), Mod.Find<ModGore>("ScourgeBody2").Type,
						1f);
					Gore.NewGore(NPC.GetSource_FromThis(null), NPC.position,
						NPC.velocity * randomSpread * Main.rand.NextFloat(), Mod.Find<ModGore>("ScourgeBody3").Type,
						1f);
				}
				for (int k = 0; k < 10; k++)
				{
