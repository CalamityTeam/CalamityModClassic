				...
				if (Main.rand.Next(300) == 0)
				{
					...
					if (Main.tile[i, tileLocationY] != null)
					{
						if (!Main.tile[i, tileLocationY].HasTile)
						{
							...
							{
								if (Main.netMode != 1)
									Projectile.NewProjectile(new EntitySource_WorldEvent(), (float)(i * 16 + 16), (float)(tileLocationY * 16 + 16), 0f, -0.1f, Mod.Find<ModProjectile>("CoralBubble").Type, 0, 0f, Main.myPlayer, 0f, 0f);
							}
						}
					}
				}
