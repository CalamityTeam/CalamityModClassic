...
namespace CalamityModClassicPreTrailer.Gores
{
    public class AstralWaterDroplet : ModGore
    {
        ...
        {
            Gore gore = new Gore();
            GoreID.Sets.DrawBehind[gore.type] = true;
            Main.instance.LoadGore(gore.type);

            int num = TextureAssets.Gore[gore.type].Value.Height / (int)gore.numFrames;

            Color alpha = gore.GetAlpha(Lighting.GetColor((int)((double)gore.position.X + (double)TextureAssets.Gore[gore.type].Value.Width * 0.5) / 16, (int)(((double)gore.position.Y + (double)num * 0.5) / 16.0)));

            Main.spriteBatch.Draw(
                TextureAssets.Gore[gore.type].Value, 
                new Vector2(gore.position.X - Main.screenPosition.X + (float)(TextureAssets.Gore[gore.type].Value.Width / 2), gore.position.Y - Main.screenPosition.Y + (float)(num / 2) - 2f), 
                new Rectangle(0, num * (int)gore.frame, TextureAssets.Gore[gore.type].Value.Width, num), 
                alpha, gore.rotation, 
                new Vector2((float)(TextureAssets.Gore[gore.type].Value.Width / 2), (float)(num / 2)), 
        ...
        public override bool Update(Gore gore)
        {
            ...
            if (gore.frame <= 4)
            {
                ...
                if ((int)gore.frameCounter >= num)
                {
                    ...
                    if (gore.frame == 5)
                    {
                        int num4 = Gore.NewGore(null, gore.position, gore.velocity, gore.type, 1f);
            ...
            if (gore.velocity != value3)
            {
                if (gore.frame < 10)
                {
                    ...
                    {
                        SoundEngine.PlaySound(in SoundID.Drip, new Vector2((int)gore.position.X + 8, (int)gore.position.Y + 8), null);
                    } 
                }
            }
