...
namespace CalamityModClassicPreTrailer.Buffs
{
	public class CalamityGlobalBuff : GlobalBuff
	{
        public override void Update(int type, Player player, ref int buffIndex)
        {
            ...
            {
                player.GetModPlayer<CalamityPlayerPreTrailer>().shine = true;
            }
            ...
            else if (type >= BuffID.NebulaUpDmg1 && type <= BuffID.NebulaUpDmg3)
            {
				...
				player.GetDamage(DamageClass.Melee) -= nebulaDamage;
				player.GetDamage(DamageClass.Ranged) -= nebulaDamage;
				player.GetDamage(DamageClass.Magic) -= nebulaDamage;
				player.GetDamage(DamageClass.Summon) -= nebulaDamage;
			}
