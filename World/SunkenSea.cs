...
{
	public class SunkenSea : ModSystem
	{
		private struct Hub
		{
			...
			for (int i = -20; i < num + 20; i++)
			{
				for (int j = -20; j < num2 + 20; j++)
				{
					...
					if (num10 > 3.5f) //Adjust num10 for all cases if you want different tile frequencies; higher is less frequent, lower is more frequent
					{
						...
						tile.WallType = (ushort)mod.Find<ModWall>("NavystoneWall").Type;
						tile.LiquidAmount = 192;
						if (num4 % 15 == 2)
						{
							tile.ResetToType((ushort)mod.Find<ModTile>("Navystone").Type);
							tile.HasTile = true;
							tile.LiquidAmount = 0;
						}
					...
					else if (num10 > 0.7f || !flag)
					{
						...
						if (!flag || tile.HasTile)
						{
							tile.ResetToType((ushort)mod.Find<ModTile>("EutrophicSand").Type);
							tile.HasTile = true;
							Tile.SmoothSlope(num6, num7, true);
							tile.LiquidAmount = 0;
						}
						tile.WallType = (ushort)mod.Find<ModWall>("EutrophicSandWall").Type;
					}
					else if (num10 > 0.25f)
					{
			...
			Generates sea prism crystals on prism ore and occasionally on navystone*/
		{
			...
			Point point = new Point(start.X + num / 2, start.Y + (int)(num2 * 0.33f)); //Around the center of the Sunken Sea area
			...
			int sunkenSeaBottom = start.Y + (int)(num2 + 0.7f);
			int smallHoles = 0;
			...
			for (int i = -20; i < num + 20; i++)
			{
				for (int j = -20; j < num2 + 20; j++)
				{
					...
					{
						genCentalHole = false;
						rectangle = new Rectangle(start.X + num / 2 - radius, start.Y + (int)(num2 * 0.33f) - radius, diameter, diameter);
						...
						WorldUtils.Gen(point, new Shapes.Circle((int)((float)radius * outerRadiusPercentage)), Actions.Chain(new GenAction[] //Smallest is 6
						{
							...
							new Actions.SetTile((ushort)mod.Find<ModTile>("Navystone").Type, true) //Place outer shell
						}));
						...
						WorldUtils.Gen(point, new Shapes.Circle((int)((float)radius * (outerRadiusPercentage * 0.6f))), Actions.Chain(new GenAction[] //Smallest is 4
						{
							...
							new Actions.SetTile((ushort)mod.Find<ModTile>("SeaPrism").Type, true) //Place prism
						}));
					...
					int smallHoleX = WorldGen.genRand.Next(start.X + 30, start.X + num2 - 30);
					...
					if (tile.TileType == mod.Find<ModTile>("EutrophicSand").Type && (!WorldGen.SolidTile(testTile) || !WorldGen.SolidTile(testTile2))) //Tile variation
					{
						tile.TileType = (ushort)mod.Find<ModTile>("Navystone").Type;
					}
				}
			}
			for (int k = -20; k < num + 20; k++)
			{
				for (int l = -20; l < num2 + 20; l++)
				{
					...
					if (tile2.HasTile && (tile2.TileType == mod.Find<ModTile>("SeaPrism").Type || tile2.TileType == mod.Find<ModTile>("Navystone").Type))
					{
						bool flag = true;
						for (int m = -1; m >= -3; m--)
						{
						...
						if (tile2.TileType == mod.Find<ModTile>("SeaPrism").Type || (tile2.TileType == mod.Find<ModTile>("Navystone").Type && WorldGen.genRand.Next(8) == 0))
						{
							if (flag3 ^ flag4)
							{
								if (tile2.Slope == 0 && !tile2.IsHalfBlock)
								{
									...
									tile3.TileType = (ushort)mod.Find<ModTile>("SeaPrismCrystals").Type;
									if (Main.tile[num5 - 1, num6].TileType == mod.Find<ModTile>("SeaPrismCrystals").Type)
									{
										Main.tile[num5 - 1, num6].TileFrameY = (short)(2 * 18);
									}
									else if (Main.tile[num5 + 1, num6].TileType == mod.Find<ModTile>("SeaPrismCrystals").Type)
									{
							...
							if (flag ^ flag2)
							{
								if (tile2.Slope == 0 && !tile2.IsHalfBlock)
								{
									...
									tile3.TileType = (ushort)mod.Find<ModTile>("SeaPrismCrystals").Type;
									if (Main.tile[num5, num6 - 1].TileType == mod.Find<ModTile>("SeaPrismCrystals").Type)
									{
										Main.tile[num5, num6 - 1].TileFrameY = (short)(0 * 18);
									}
									else if (Main.tile[num5, num6 + 1].TileType == mod.Find<ModTile>("SeaPrismCrystals").Type)
									{
					...
					if (!tile2.HasTile)
					{
						if (tile2.WallType == mod.Find<ModWall>("NavystoneWall").Type || tile2.WallType == mod.Find<ModWall>("EutrophicSandWall").Type)
						{
							if (WorldGen.genRand.Next(5) == 0)
							{
								WorldGenerationMethods.PlaceTit(num5, num6, (ushort)mod.Find<ModTile>("SunkenSeaStalactite").Type);
							}
							if (WorldGen.genRand.Next(8) == 0)
							{
								WorldGen.PlaceTile(num5, num6, (ushort)mod.Find<ModTile>("BrainCoral").Type, true, false, -1, 0);
							}
							if (WorldGen.genRand.Next(6) == 0)
							{
								WorldGen.PlaceTile(num5, num6, (ushort)mod.Find<ModTile>("SmallBrainCoral").Type, true, false, -1, 0);
							}
							if (WorldGen.genRand.Next(10) == 0)
							{
								WorldGen.PlaceTile(num5, num6, (ushort)mod.Find<ModTile>("FanCoral").Type, true, false, -1, 0);
							}
							if (WorldGen.genRand.Next(6) == 0)
							{
								WorldGen.PlaceTile(num5, num6, (ushort)mod.Find<ModTile>("SeaAnemone").Type, true, false, -1, 0);
							}
							if (WorldGen.genRand.Next(8) == 0)
							{
								WorldGen.PlaceTile(num5, num6, (ushort)mod.Find<ModTile>("TubeCoral").Type, true, false, -1, 0);
							}
							if (WorldGen.genRand.Next(6) == 0)
							{
								WorldGen.PlaceTile(num5, num6, (ushort)mod.Find<ModTile>("SmallTubeCoral").Type, true, false, -1, 0);
							}
							if (WorldGen.genRand.Next(4) == 0)
							{
								WorldGen.PlaceTile(num5, num6, (ushort)mod.Find<ModTile>("TableCoral").Type, true, false, -1, 0);
							}
						}
					}
				}
