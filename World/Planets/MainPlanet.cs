...
using CalamityModClassicPreTrailer.MiscImplementation;
using Terraria.WorldBuilding;

namespace CalamityModClassicPreTrailer.World.Planets
{
    public class MainPlanet : Planetoid
    {
        ...
        public void PlacePlanet(Point origin, int radius)
        {
            ...
            ushort LabWall = WallID.StoneSlab;
            ...
            {
                new Actions.SetTile((ushort)CalamityModClassicPreTrailer.Instance.Find<ModTile>("Cinderplate").Type, true).Output(cinderPlateBlob),
                new Actions.ClearWall(),
            ...
            foreach(Point16 p in bottomMostTiles)
            {
                ...
                if (!_tiles[tileX, tileY + 1].HasTile && tileX % doPillarEveryXBlocks == 0)
                {
            ...
            for (int x = origin.X - corridorLength; x <= origin.X + corridorLength; x++)
            {
                for (int y = origin.Y - 4; y <= origin.Y + 4; y++)
                {
                    ...
                    if (x == origin.X - corridorLength || x == origin.X + corridorLength)
                    {
                        ...
                        _tiles[x, y].TileType = LabTile;
                    }
                    else if (y == origin.Y - 4 || y == origin.Y + 4)
                    {
                        bool bottom = y == origin.Y + 4;
                        if (distFromOriginX < cinderRadius + 9 && !_tiles[x, y].HasTile)
                        {
                            _tiles[x, y].Get<TileWallWireStateData>().HasTile = false;
                        ...
                        else
                        {
                            ...
                            if (bottom && !_tiles[x, y + 1].HasTile && x % doPillarEveryXBlocks == 0)
                            {
            ...
            {
                Main.tile[doorX, y3].Get<TileWallWireStateData>().HasTile = true;
                Main.tile[doorX, y3].TileType = TileID.StoneSlab;
            }
            ...
            if (labLeftSide)
            {
                ...
                {
                    Main.tile[x, origin.Y].Get<TileWallWireStateData>().RedWire = true;
                    if (x == doorX - 7)
                    {
                        for (int y = origin.Y - 2; y < origin.Y; y++)
                        {
                            Main.tile[x, y].Get<TileWallWireStateData>().RedWire = true;
                        }
                    }
                }
            }
            ...
            {
                _tiles[lanternX, y].TileFrameX = 18;
            }
            ...
            {
                Main.tile[entranceDoorX, i].Get<TileWallWireStateData>().HasTile = false;
            }
        ...
        public void PlaceBeams(ushort beamType, int x, int startY, Vector2 planetCenter, int radius, params ushort[] ignoreWalls)
        {
            ...
            while (!_tiles[x, startY].HasTile && !walls.Contains(_tiles[x, startY].WallType))
            {
                ...
                _tiles[x, startY].TileType = beamType;
                startY++;
