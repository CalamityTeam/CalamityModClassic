    ...
    public class PlasmaRay : ModProjectile
    {
        ...
        public override void AI()
        {
			...
			if (Projectile.localAI[0] > 9f)
			{
				for (int num447 = 0; num447 < 4; num447++)
				{
					...
					int num448 = Dust.NewDust(vector33, 1, 1, 173, 0f, 0f, 0, default(Color), 0.75f);
        ...
        public override bool OnTileCollide(Vector2 oldVelocity)
        {
			...
			for (i = 0; i < 4; i++ )
			{
				...
				Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, (float)( Math.Sin(offsetAngle) * 5f ), (float)( Math.Cos(offsetAngle) * 5f ), Mod.Find<ModProjectile>("PlasmaRay2").Type, (int)((double)Projectile.damage * 1.5f), Projectile.knockBack, Projectile.owner, 0f, 0f);
				Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, (float)( -Math.Sin(offsetAngle) * 5f ), (float)( -Math.Cos(offsetAngle) * 5f ), Mod.Find<ModProjectile>("PlasmaRay2").Type, (int)((double)Projectile.damage * 1.5f), Projectile.knockBack, Projectile.owner, 0f, 0f);
			}
