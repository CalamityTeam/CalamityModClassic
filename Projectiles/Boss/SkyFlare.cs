        ...
        public override void OnKill(int timeLeft)
        {
			...
			if (Projectile.owner == Main.myPlayer)
			{
				...
				if (tornadoType < 5)
				{
					...
					int num236 = Projectile.NewProjectile(Projectile.GetSource_FromThis(null),(float)(num232 * 16 + 8), (float)(num231 * 16 - 24), 0f, 0f, Mod.Find<ModProjectile>("Flarenado").Type, num235, 4f, Main.myPlayer, 16f, 15f + (revenge ? 4f : 0f));
					Main.projectile[num236].netUpdate = true;
				}
				else
				{
					int num235 = Main.expertMode ? 230 : 400; //920
					int num236 = Projectile.NewProjectile(Projectile.GetSource_FromThis(null),(float)(num232 * 16 + 8), (float)(num231 * 16 - 24), 0f, 0f, Mod.Find<ModProjectile>("Infernado").Type, num235, 4f, Main.myPlayer, 16f, 16f + (revenge ? 4f : 0f));
					Main.projectile[num236].netUpdate = true;
				}
			}
        }
