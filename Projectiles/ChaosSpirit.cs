...
namespace CalamityModClassic1Point2.Projectiles
{
    public class ChaosSpirit : ModProjectile
    {
        ...
        {
        	bool flag64 = Projectile.type == Mod.Find<ModProjectile>("ChaosSpirit").Type;
        	...
        	if (dust >= 0)
        	{
				...
				{
					int num503 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y + 16f), Projectile.width, Projectile.height - 16, DustID.Flare, 0f, 0f, 0, default(Color), 1f);
        	...
        	if (Projectile.owner == Main.myPlayer)
			{
				...
				if (flag18)
				{
					...
					int projectileType = Mod.Find<ModProjectile>("ChaosFlame").Type;
					...
					num404 *= num406;
					num405 *= num406;
					Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X - 4f, Projectile.Center.Y, num404, num405, projectileType, (int)((double)190 * player.GetDamage(DamageClass.Summon).Multiplicative), 5f, Projectile.owner, 0f, 0f);
					Projectile.ai[0] = 38f;
					return;
				}
			}
