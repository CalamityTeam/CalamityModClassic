...
namespace CalamityModClassic1Point2.Projectiles
{
    public class MeowFire : ModProjectile
    {
			...
			if (Projectile.ai[0] > 7f)
			{
				...
				if (Main.rand.NextBool(2))
				{
					for (int num298 = 0; num298 < 2; num298++)
					{
						...
						if ((num297 == 56 && Main.rand.NextBool(3)))
						{
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
			...
			{
				int num622 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, DustID.BlueFairy, 0f, 0f, 100, default(Color), 1f);
			...
			{
				int num624 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, DustID.BlueFairy, 0f, 0f, 100, default(Color), 1.5f);
				Main.dust[num624].noGravity = true;
				Main.dust[num624].velocity *= 5f;
				num624 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, DustID.BlueFairy, 0f, 0f, 100, default(Color), 1f);
