			...
			if (flag10 && Main.myPlayer == Projectile.owner)
			{
				...
				if (flag12)
				{
					if (Projectile.ai[0] == 1f)
					{
						...
						for (int l = 0; l < 7; l++)
						{
							Projectile.NewProjectile(Projectile.GetSource_FromThis(null), center3.X, center3.Y, vector12.X, vector12.Y, Mod.Find<ModProjectile>("DarkSparkBeam").Type, num29, Projectile.knockBack, Projectile.owner, (float)l, (float)Projectile.whoAmI);
						}
						Projectile.netUpdate = true;
					}
				}
