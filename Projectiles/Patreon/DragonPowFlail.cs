...
namespace CalamityModClassicPreTrailer.Projectiles.Patreon
{
    public class DragonPowFlail : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                int dustType = (Main.rand.Next(3) == 0) ? 246 : 244;
            ...
            {
                int type = Mod.Find<ModProjectile>("DraconicSpark").Type;
                ...
                if(Projectile.owner == Main.myPlayer)
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(null), Projectile.Center, vel, type, damage, kb, Projectile.owner, sparkVariety, 0f);
            }
        }
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            target.AddBuff(Mod.Find<ModBuff>("AbyssalFlames").Type, 480);
            target.AddBuff(Mod.Find<ModBuff>("HolyLight").Type, 480);
            ...
            PetalStorm(target, target.damage, hit.Knockback, hit.Crit);
            Waterfalls(target, target.damage, hit.Knockback, hit.Crit);
        }
        ...
        private void PetalStorm(NPC target, int damage, float knockback, bool crit)
        {
            ...
            for (int i = 0; i < numPetals; ++i)
            {
                ...
                if (Projectile.owner == Main.myPlayer)
                {
                    int idx = Projectile.NewProjectile(Projectile.GetSource_FromThis(null), startPos, petalVel, type, petalDamage, petalKB, Projectile.owner, 0f, 0f);
        ...
        {
            int type = Mod.Find<ModProjectile>("OceanBeam").Type;
            ...
            for(int i = 0; i < numWaterfalls; ++i)
            {
                ...
                if (Projectile.owner == Main.myPlayer)
                {
                    int idx = Projectile.NewProjectile(Projectile.GetSource_FromThis(null), startPos, fallVec, type, waterfallDamage, waterfallKB, Projectile.owner, 0f, 0f);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D chainTex = ModContent.Request<Texture2D>("CalamityModClassicPreTrailer/ExtraTextures/Chains/DragonPowChain").Value;
