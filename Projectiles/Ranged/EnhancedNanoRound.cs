        	...
        	if (target.life <= 0)
        	{
	        	if (Projectile.owner == Main.myPlayer)
	        	{
					for (int num252 = 0; num252 < 2; num252++)
					{
						...
						Projectile.NewProjectile(Projectile.GetSource_FromThis(null), Projectile.oldPosition.X + (float)(Projectile.width / 2), Projectile.oldPosition.Y + (float)(Projectile.height / 2), value15.X, value15.Y, Mod.Find<ModProjectile>("Nanomachine").Type, (int)((double)Projectile.damage * 0.3), 0f, Projectile.owner, 0f, 0f);
					}
	        	}
        	}
        }
