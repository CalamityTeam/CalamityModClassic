	...
	public class SicknessRound : ModProjectile
	{
		...
		public override bool PreAI()
        {
			for (int num136 = 0; num136 < 7; num136++)
			{
				...
				int num137 = Dust.NewDust(new Vector2(x2, y2), 1, 1, 107, 0f, 0f, 0, default(Color), 1f);
		...
		public override void OnKill(int timeLeft)
        {
			if (Projectile.owner == Main.myPlayer)
			{
	            ...
	            for (int k = 0; k < 3; k++)
	            {
	            	...
	            	Projectile.NewProjectile(Projectile.GetSource_FromThis(null), Projectile.position.X, Projectile.position.Y, (float)Main.rand.Next(-35, 36) * 0.2f, (float)Main.rand.Next(-35, 36) * 0.2f, Mod.Find<ModProjectile>("SicknessRound2").Type, 
