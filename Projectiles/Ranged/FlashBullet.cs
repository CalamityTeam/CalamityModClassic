...
namespace CalamityModClassicPreTrailer.Projectiles.Ranged
{
	public class FlashBullet : ModProjectile
	{
		...
		public override void AI()
		{
        	...
        	if (Projectile.localAI[0] >= 6f)
        	{
				for (int num136 = 0; num136 < 10; num136++)
				{
					...
					int num137 = Dust.NewDust(new Vector2(x2, y2), 1, 1, 264, 0f, 0f, 0, default(Color), 0.5f);
		...
		public override void OnKill(int timeLeft)
        {
        	...
        	{
        		Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, 0f, 0f, Mod.Find<ModProjectile>("Flash").Type, (int)((double)Projectile.damage * 0.25), 0f, Projectile.owner, 0f, 0f);
        	}
            for (int k = 0; k < 5; k++)
            {
