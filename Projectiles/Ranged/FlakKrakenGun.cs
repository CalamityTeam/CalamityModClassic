...
namespace CalamityModClassicPreTrailer.Projectiles.Ranged
{
    public class FlakKrakenGun : ModProjectile
    {
        ...
        public override void AI()
        {
			...
			Projectile.damage = (int)((float)player.inventory[player.selectedItem].damage * player.GetDamage(DamageClass.Ranged).Multiplicative);
			Projectile.ai[0] += 1f;
			Projectile.ai[1] += 1f;
			int num27 = 10;
			bool flag10 = false;
			...
			if (flag10 && Main.myPlayer == Projectile.owner)
			{
				...
				if (flag12)
				{
					if (Projectile.ai[0] == 1f)
					{
						...
						Projectile.NewProjectile(Entity.GetSource_FromThis(null), vector2.X, vector2.Y, 0f, 0f, Mod.Find<ModProjectile>("FlakKraken").Type, num29, Projectile.knockBack, Projectile.owner, 0f, (float)Projectile.whoAmI);
						Projectile.netUpdate = true;
					}
				}
				else
