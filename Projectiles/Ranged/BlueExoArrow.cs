    ...
    public class BlueExoArrow : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            for (int x = 0; x < 3; x++)
            {
                ...
                {
                    Projectile.NewProjectile(Entity.GetSource_FromThis(null), vector2.X, vector2.Y, speedX, speedY, Mod.Find<ModProjectile>("BlueExoArrow2").Type, (int)((double)Projectile.damage * 0.7f), 1f, Projectile.owner);
                }
            }
        }
    }
