...
namespace CalamityModClassicPreTrailer.Projectiles.Pets
{
	public class ChibiiDoggo : ModProjectile
	{
		...
		public override bool PreDraw (ref Color lightColor)
		{
			...
			Texture2D texture2D3 = ModContent.Request<Texture2D>("CalamityModClassicPreTrailer/Projectiles/Pets/ChibiiDoggoMonochrome").Value;
			...
			Main.spriteBatch.Draw(TextureAssets.Projectile[Projectile.type].Value, Projectile.position + Projectile.Size / 2f - Main.screenPosition + new Vector2(0f, Projectile.gfxOffY), new Microsoft.Xna.Framework.Rectangle?(rectangle), lightColor, Projectile.rotation, origin2, Projectile.scale, spriteEffects, 0f);
			return false;
		}
		...
		public void SpawnDoggo()
		{
			...
			int n = NPC.NewNPC(Projectile.GetSource_FromThis(null), (int)Projectile.position.X, (int)Projectile.position.Y, type);
		...
		public override void AI()
		{
			...
			else
			{
				...
				if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("ChibiiDoggoFly").Type] <= 0)
					Projectile.NewProjectile(Projectile.GetSource_FromThis(null), Projectile.position.X, Projectile.position.Y, 0f, 0f, Mod.Find<ModProjectile>("ChibiiDoggoFly").Type, 0, 0f, Projectile.owner, (float) Projectile.identity);
			}
			...
			if ((double) notlocalai1 > (double) Main.rand.Next(30, 120) && !player.immune && player.velocity.X == 0 && player.velocity.Y == 0)
            {
				if (Main.rand.Next(3) == 0)
				{
					...
					{
						SoundEngine.PlaySound(SoundID.ScaryScream.WithVolumeScale(2f), player.position); //REEEEEEE
					}
					notlocalai1 = -600f;
				}
				else
				{
					...
					player.Hurt(PlayerDeathReason.ByOther(6), 500, 0, false, false, 0, false);
					player.immune = false;
					player.immuneTime = 0;
