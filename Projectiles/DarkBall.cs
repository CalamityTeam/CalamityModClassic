...
namespace CalamityModClassic1Point1.Projectiles
{
    public class DarkBall : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
			...
			{
				int num649 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, 14, 0f, 0f, 100, default(Color), 1.5f);
				Main.dust[num649].velocity *= 1.4f;
			}
			for (int num650 = 0; num650 < 10; num650++)
			{
				int num651 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, 14, 0f, 0f, 100, default(Color), 2.5f);
