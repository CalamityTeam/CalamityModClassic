...
namespace CalamityModClassic1Point2.Projectiles
{
	public class GhastlyExplosion : ModProjectile
    {
        ...
        public override void AI()
        {
			...
			{
				int num330 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, DustID.RedTorch, Projectile.velocity.X, Projectile.velocity.Y, num328, default(Color), 1.2f);
			...
			{
				int num330 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, DustID.RedTorch, Projectile.velocity.X, Projectile.velocity.Y, num328, default(Color), 0.4f);
				if (num331 == 0) 
				{
        ...
        public override void OnKill(int timeLeft)
        {
			...
			if (Main.myPlayer == Projectile.owner)
			{
				...
				for (int num119 = 0; num119 < num118; num119 = num3 + 1)
				{
					...
					Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.oldPosition.X + (float)(Projectile.width / 2), Projectile.oldPosition.Y + (float)(Projectile.height / 2), vector8.X, vector8.Y, Mod.Find<ModProjectile>("GhastlyExplosionShard").Type, (int)((double)Projectile.damage * 0.9), Projectile.knockBack * 0.8f, Projectile.owner, Projectile.ai[0], 0f);
					num3 = num119;
				}
			}
        }
