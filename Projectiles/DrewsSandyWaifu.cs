...
namespace CalamityModClassic1Point2.Projectiles
{
    public class DrewsSandyWaifu : ModProjectile
    {
		...
		{
			//DisplayName.SetDefault("Drew's Waifu");
			...
			if (Projectile.ai[0] == 0f)
			{
				...
				if (flag25 && Projectile.ai[1] == 0f)
				{
					...
					if (Main.myPlayer == Projectile.owner && Collision.CanHitLine(Projectile.position, Projectile.width, Projectile.height, vector46, 0, 0))
					{
						...
						int num659 = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, value19.X, value19.Y, num658, (int)((float)Projectile.damage * 1.15f), 0f, Main.myPlayer, 0f, 0f);
