...
namespace CalamityModClassicPreTrailer.Projectiles.Typeless
{
	public class GoldenGun : ModProjectile
	{
		...
		public override void AI()
        {
            ...
            for (int num586 = 0; num586 < 1; num586 = num3 + 1)
            {
                for (int num587 = 0; num587 < 6; num587 = num3 + 1)
                {
                    ...
                    int num590 = 6;
                    int num591 = Dust.NewDust(new Vector2(Projectile.position.X + (float)num590, Projectile.position.Y + (float)num590), Projectile.width - num590 * 2, Projectile.height - num590 * 2, 170, 0f, 0f, 75, default(Color), 1.2f);
                    Dust dust;
                ...
                {
                    int num592 = 6;
                    int num593 = Dust.NewDust(new Vector2(Projectile.position.X + (float)num592, Projectile.position.Y + (float)num592), Projectile.width - num592 * 2, Projectile.height - num592 * 2, 170, 0f, 0f, 75, default(Color), 0.65f);
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            {
                int num363 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, 170, 0f, 0f, 75, default(Color), 1.2f);
                Dust dust;
                if (Main.rand.Next(2) == 0)
                {
