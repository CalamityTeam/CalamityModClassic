...
namespace CalamityModClassic1Point2.Projectiles
{
	public class EnhancedNanoRound : ModProjectile
	{
		...
		public override void AI()
		{
        	...
        	if (Projectile.localAI[0] >= 6f)
        	{
				for (int num136 = 0; num136 < 10; num136++)
				{
					...
					int num137 = Dust.NewDust(new Vector2(x2, y2), 1, 1, DustID.Vortex, 0f, 0f, 0, default(Color), 0.5f);
        	...
        	if (target.life <= 0)
        	{
	        	...
	        	if (Projectile.owner == Main.myPlayer)
	        	{
					for (int num252 = 0; num252 < num251; num252++)
					{
						...
						Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.oldPosition.X + (float)(Projectile.width / 2), Projectile.oldPosition.Y + (float)(Projectile.height / 2), value15.X, value15.Y, Mod.Find<ModProjectile>("Nanomachine").Type, (int)((double)Projectile.damage), 0f, Projectile.owner, 0f, 0f);
					}
	        	}
        	}
        }
		...
		public override void OnKill(int timeLeft)
        {
			...
			{
				int num214 = Dust.NewDust(Projectile.Center - Projectile.velocity / 2f, 0, 0, DustID.Vortex, 0f, 0f, 100, default(Color), 2f);
