    ...
    public class CosmicDischarge : ModProjectile
    {
        ...
        public override void AI()
        {
			...
			if (Projectile.alpha == 0)
			{
				...
				{
					Dust dust = Main.dust[Dust.NewDust(Projectile.position + Projectile.velocity * 2f, Projectile.width, Projectile.height, DustID.IceRod, 0f, 0f, 100, new Color(150, Main.DiscoG, 255), 2f)];
					dust.noGravity = true;
					dust.velocity *= 2f;
				...
				while ((float)num53 < num52)
				{
					if (Main.rand.NextBool(4))
					{
						...
						Dust dust2 = Main.dust[Dust.NewDust(position, Projectile.width, Projectile.height, DustID.Flare_Blue, 0f, 0f, 100, new Color(150, Main.DiscoG, 255), 1f)];
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
			...
			{
				Projectile.NewProjectile(Projectile.GetSource_FromThis(), target.Center.X, target.Center.Y, 0f, 0f, Mod.Find<ModProjectile>("CosmicIceBurst").Type, hit.Damage, 10f, Projectile.owner, 0f, 0.85f + Main.rand.NextFloat() * 1.15f);
			}
			Projectile.localAI[1] = 4f;
			target.AddBuff(Mod.Find<ModBuff>("CosmicFreeze").Type, 300);
        }
    }
}