    ...
    public class ThunderstormShotSplit : ModProjectile
    {
        ...
        public override void AI()
        {
			...
			if (Projectile.ai[0] > 6f)
			{
				...
				{
					Dust dust = Main.dust[Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, 107, Projectile.velocity.X, Projectile.velocity.Y, 100, default(Color), 0.5f)];
					dust.velocity = Vector2.Zero;
					dust.position -= Projectile.velocity / 5f * (float)num121;
					dust.noGravity = true;
					dust.noLight = true;
					Dust dust2 = Main.dust[Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, 229, Projectile.velocity.X, Projectile.velocity.Y, 100, default(Color), 0.5f)];
        ...
        public override void OnKill(int timeLeft)
        {
			...
			for (int num227 = 0; num227 < num226; num227++)
			{
				...
				int num228 = Dust.NewDust(vector6 + vector7, 0, 0, 229, vector7.X, vector7.Y, 100, default(Color), 0.6f);
			...
			for (int num227 = 0; num227 < num226; num227++)
			{
				...
				vector6 = vector6.RotatedBy((double)((float)(num227 - (num226 / 2 - 1)) * 6.28318548f / (float)num226), default(Vector2)) + Projectile.Center;
				Vector2 vector7 = vector6 - Projectile.Center;
				int num228 = Dust.NewDust(vector6 + vector7, 0, 0, 107, vector7.X, vector7.Y, 100, default(Color), 0.6f);
				Main.dust[num228].noGravity = true;
				Main.dust[num228].noLight = true;
				Main.dust[num228].velocity = vector7;
			}
