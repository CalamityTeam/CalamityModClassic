...
namespace CalamityModClassic1Point2.Projectiles
{
    public class StormMarkSummon : ModProjectile
    {
        ...
        public override void AI()
        {
			...
			if (Projectile.localAI[1] < 30f) 
			{
				for (int num1134 = 0; num1134 < 1; num1134++) 
				{
					...
					Dust dust34 = Main.dust[Dust.NewDust(position4, 0, 0, DustID.Flare_Blue, 0f, 0f, 0, default(Color), 0.5f)];
			...
			for (int num1135 = 0; num1135 < 2; num1135++) 
			{
				if (Main.rand.NextBool(10)) 
				{
					...
					Dust dust35 = Main.dust[Dust.NewDust(Projectile.Center, 0, 0, DustID.Flare_Blue, 0f, 0f, 225, newColor3, 1f)];
			...
			for (int num1136 = 0; num1136 < 2; num1136++) 
			{
				if (Main.rand.NextBool(10)) 
				{
					...
					Dust dust36 = Main.dust[Dust.NewDust(Projectile.Center, 0, 0, DustID.Flare_Blue, 0f, 0f, 225, newColor3, 1.5f)];
			...
			{
				int storm = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Vector2.Zero, Mod.Find<ModProjectile>("Tornado").Type, Projectile.damage, 2f, Projectile.owner, 1f, 0f);
				Main.projectile[storm].minion = true;
				Main.projectile[storm].DamageType = DamageClass.Summon;
			}
        ...
        public override bool PreDraw(ref Color lightColor)
        {
			...
			Main.EntitySpriteDraw(texture2D27, vector38, new Microsoft.Xna.Framework.Rectangle?(rectangle11), color47, 0f, origin7, new Vector2(1f, 5f) * Projectile.scale * 2f, spriteEffects, 0f);
			Main.EntitySpriteDraw(texture2D27, vector38, new Microsoft.Xna.Framework.Rectangle?(rectangle11), alpha5, Projectile.rotation, origin7, Projectile.scale, spriteEffects, 0f);
			Main.EntitySpriteDraw(texture2D27, vector38, new Microsoft.Xna.Framework.Rectangle?(rectangle11), alpha5, 0f, origin7, new Vector2(1f, 8f) * Projectile.scale, spriteEffects, 0f);
			return false;
        }
    }
}