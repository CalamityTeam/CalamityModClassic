					...
					if (Main.rand.Next(3) == 0)
					{
						if (Projectile.owner == Main.myPlayer)
						{
					    	...
					    	for (i = 0; i < 4; i++ )
					    	{
					        	...
					        	Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, (float)( Math.Sin(offsetAngle) * 5f ), (float)( Math.Cos(offsetAngle) * 5f ), Mod.Find<ModProjectile>("AccretionDisk2Melee").Type, Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
					        	Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, (float)( -Math.Sin(offsetAngle) * 5f ), (float)( -Math.Cos(offsetAngle) * 5f ), Mod.Find<ModProjectile>("AccretionDisk2Melee").Type, Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
					    	}
						}
					}
