    ...
    public class Mourningstar : ModProjectile
    {
        ...
        public override void AI()
        {
			...
			if (Projectile.alpha == 0)
			{
				...
				{
					Dust dust = Main.dust[Dust.NewDust(Projectile.position + Projectile.velocity * 2f, Projectile.width, Projectile.height, 244, 0f, 0f, 100, new Color(255, 200, 0), 2f)];
					dust.noGravity = true;
					dust.velocity *= 2f;
				...
				while ((float)num53 < num52)
				{
					if (Main.rand.Next(4) == 0)
					{
						...
						Dust dust2 = Main.dust[Dust.NewDust(position, Projectile.width, Projectile.height, 246, 0f, 0f, 100, new Color(255, 200, 0), 1f)];
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
			...
			{
				Projectile.NewProjectile(Projectile.GetSource_FromThis(null), target.Center.X, target.Center.Y, 0f, 0f, 612, hit.Damage, 10f, Projectile.owner, 0f, 0.85f + Main.rand.NextFloat() * 1.15f);
			}
