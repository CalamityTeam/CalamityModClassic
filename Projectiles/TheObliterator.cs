        ...
        public override void AI()
        {
			...
			if (flag14)
			{
				...
				if (Projectile.localAI[0] > 16f)
				{
					...
					num438 *= num440;
					num439 *= num440;
					int projChoice = Main.rand.Next(2);
					if (projChoice == 0)
					{
						projChoice = Mod.Find<ModProjectile>("NebulaShot").Type;
					}
					else
					{
						projChoice = Mod.Find<ModProjectile>("IceBeam").Type;
					}
					if (Projectile.owner == Main.myPlayer)
					{
						int shoot = Projectile.NewProjectile(Projectile.GetSource_FromThis(), value10.X, value10.Y, num438, num439, projChoice, (int)((double)Projectile.damage * 0.75f), Projectile.knockBack, Projectile.owner, 0f, 0f);
						Main.projectile[shoot].DamageType = DamageClass.Melee;
					}
					return;
				}
			}
