	...
	public class ClamCrusherFlail : ModProjectile
	{
		...
		public override bool OnTileCollide(Vector2 oldVelocity)
		{
			if (Projectile.ai[1] >= 5f)
			{
				...
				SoundEngine.PlaySound(new SoundStyle("CalamityModClassicPreTrailer/Sounds/Item/ClamImpact"), Projectile.position);
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			Texture2D texture2D2 = ModContent.Request<Texture2D>("CalamityModClassicPreTrailer/ExtraTextures/Chains/ClamCrusherChain").Value;
		...
		public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
		{
			if (!target.boss)
			{
				if (target.rarity != 2)
				{
					target.AddBuff(Mod.Find<ModBuff>("SilvaStun").Type, 60);
				}
				else if (Projectile.ai[1] >= 45f && (Projectile.ai[0] != 1f || Projectile.ai[0] != 2f))
				{
					target.AddBuff(Mod.Find<ModBuff>("SilvaStun").Type, 30);
				}
			}
			Projectile.ai[0] = 1f;
			Projectile.netUpdate = true;
