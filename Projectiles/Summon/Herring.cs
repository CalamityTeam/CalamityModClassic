...
namespace CalamityModClassicPreTrailer.Projectiles.Summon
{
    public class Herring : ModProjectile
    {
        ...
        public override void AI()
        {
			...
			float num635 = 1200f;
			float num636 = 150f;
			bool flag64 = Projectile.type == Mod.Find<ModProjectile>("Herring").Type;
			Player player = Main.player[Projectile.owner];
			CalamityPlayerPreTrailer modPlayer = player.GetModPlayer<CalamityPlayerPreTrailer>();
            player.AddBuff(Mod.Find<ModBuff>("Herring").Type, 3600);
            if (flag64)
			{
				if (player.dead)
				{
			...
			{
				bool flag23 = (Main.projectile[num638].type == Mod.Find<ModProjectile>("Herring").Type);
