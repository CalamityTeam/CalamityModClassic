...
namespace CalamityModClassicPreTrailer.Projectiles.Summon
{
    public class SolarGod : ModProjectile
    {
        ...
        public override void AI()
        {
			...
			player.AddBuff(Mod.Find<ModBuff>("SolarSpiritGod").Type, 3600);
			if (flag64)
			{
				if (player.dead)
				{
			...
			if (Projectile.owner == Main.myPlayer)
			{
				...
				if (flag11)
				{
					...
					num404 *= num406;
					num405 *= num406;
					Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X - 4f, Projectile.Center.Y, num404, num405, Mod.Find<ModProjectile>("SolarBeam").Type, Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
					Projectile.ai[0] = 30f;
				}
			}
        }
