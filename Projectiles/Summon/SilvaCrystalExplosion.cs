...
namespace CalamityModClassicPreTrailer.Projectiles.Summon
{
    public class SilvaCrystalExplosion : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (num978 < 0 || num978 >= 1000 || (!Main.projectile[num978].active && Main.projectile[num978].type != Mod.Find<ModProjectile>("SilvaCrystal").Type))
            {
                Projectile.ai[1] = -1f;
            }
            else
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (Projectile.ai[1] != -1f && Projectile.Opacity > 0.3f)
            {
                ...
                if (num279 > 0f)
                {
                    Main.spriteBatch.Draw(texture2D34, vector59 + vector61 / 2f, new Microsoft.Xna.Framework.Rectangle?(rectangle17), color60 * num279, rotation27, origin11, vector62, SpriteEffects.None, 0f);
                    Main.spriteBatch.Draw(texture2D34, vector59 + vector61 / 2f, new Microsoft.Xna.Framework.Rectangle?(rectangle17), alpha5 * num279, rotation27, origin11, vector62 / 2f, SpriteEffects.None, 0f);
                }
            }
            return false;
        }
