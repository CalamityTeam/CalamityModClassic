    ...
    public class DreadmineTurret : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Main.rand.Next(15) == 0)
            {
                ...
                {
                    if (Main.projectile[i].active && Main.projectile[i].owner == Main.myPlayer && Main.projectile[i].type == Mod.Find<ModProjectile>("Dreadmine").Type)
                    {
                        num++;
                    }
                }
                                ...
                                if (flag)
                                {
                                    for (int k = 0; k < 1000; k++)
                                    {
                                        if (Main.projectile[k].active && Main.projectile[k].owner == Main.myPlayer && 
                                            Main.projectile[k].type == Mod.Find<ModProjectile>("Dreadmine").Type && (center - Main.projectile[k].Center).Length() < 48f)
                                        {
                                            flag = false;
                                            break;
                                        }
                                    ...
                                    {
                                        Projectile.NewProjectile(Projectile.GetSource_FromThis(null), center.X, center.Y, 0f, 0f, Mod.Find<ModProjectile>("Dreadmine").Type, Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
                                    }
                                }
                            }
                        }
