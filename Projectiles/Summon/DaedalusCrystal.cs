    ...
    public class DaedalusCrystal : ModProjectile
    {
        ...
        public override void AI()
        {
        	...
        	if (dust >= 0)
        	{
				...
				{
					int num503 = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y + 16f), Projectile.width, Projectile.height - 16, 173, 0f, 0f, 0, default(Color), 1f);
        	...
        	if (Projectile.owner == Main.myPlayer)
			{
				...
				if (flag18)
				{
					...
					num404 *= num406;
					num405 *= num406;
					Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X - 4f, Projectile.Center.Y, num404, num405, projectileType, Projectile.damage, 5f, Projectile.owner, 0f, 0f);
					Projectile.ai[0] = 50f;
				}
			}
        }
