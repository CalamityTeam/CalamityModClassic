...
namespace CalamityModClassic1Point1.Projectiles
{
	public class AstralRound : ModProjectile
	{
		...
		{
			//Tooltip.SetDefault("Astral Round");
			Projectile.width = 8;
			Projectile.height = 8;
		...
		public override bool PreAI()
        {
			for (int num136 = 0; num136 < 3; num136++)
			{
				...
				int num137 = Dust.NewDust(new Vector2(x2, y2), 1, 1, 87, 0f, 0f, 0, default(Color), 1.5f);
			...
			for (int num136 = 0; num136 < 3; num136++)
			{
				float x2 = Projectile.position.X - Projectile.velocity.X / 10f * (float)num136;
				float y2 = Projectile.position.Y - Projectile.velocity.Y / 10f * (float)num136;
				int num137 = Dust.NewDust(new Vector2(x2, y2), 1, 1, 86, 0f, 0f, 0, default(Color), 1.5f);
				Main.dust[num137].alpha = Projectile.alpha;
				Main.dust[num137].position.X = x2;
				Main.dust[num137].position.Y = y2;
				Main.dust[num137].velocity *= 0f;
